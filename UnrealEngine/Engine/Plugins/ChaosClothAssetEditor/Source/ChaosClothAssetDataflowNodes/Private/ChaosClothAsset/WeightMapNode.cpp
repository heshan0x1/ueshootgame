	...
	if (ClothFacade.IsValid())  // Can only act on the collection if it is a valid cloth collection
	{
		...
		if (InTransferCollection.IsEmpty())
		{
			FClothDataflowTools::LogAndToastWarning(*this, 
				LOCTEXT("OnTransferError_TransferError", "Transfer Error"),
				LOCTEXT("OnTransferError_TransferCollectionEmpty", "Transfer collection is empty, no source data to transfer from")
			);
			return;
		}

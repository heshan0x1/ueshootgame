...
#include UE_INLINE_GENERATED_CPP_BY_NAME(PCGWorldPartitionBuilder)

DEFINE_LOG_CATEGORY_STATIC(LogPCGWorldPartitionBuilder, Verbose, All);
...
struct FPCGWorldPartitionBuilder
{
	...
	static TArray<UPackage*> GetDirtyPackages(const UWorld* InWorld, const TSet<FString>& AllowedAssetPackages);
	...
	static void GetRevertableActorsFromComponents(TArray<TWeakObjectPtr<UPCGComponent>> InComponentsToGenerate, const FPCGWorldPartitionBuilderArgs& InArgs, TMap<TSoftObjectPtr<AActor>, FPCGCrc>& OutRevertableActors);
	static void GetRevertablePartitionActors(UWorld* InWorld, const FPCGWorldPartitionBuilderArgs& InArgs, TMap<TSoftObjectPtr<AActor>, FPCGCrc>& OutRevertableActors);
	static void GetRevertableActorsFromDirtyPackages(TArray<UPackage*> InDirtyPackages, const FPCGWorldPartitionBuilderArgs& InArgs, TMap<TSoftObjectPtr<AActor>, FPCGCrc>& OutRevertableActors);
	static void RevertUnchangedActors(TArray<UPackage*>& InOutDirtyPackages, const FPCGWorldPartitionBuilderArgs& InArgs, const TMap<TSoftObjectPtr<AActor>, FPCGCrc>& InRevertableActors, TSet<TSoftObjectPtr<AActor>>& InOutRevertedActors);
	static bool IsActorRevertable(AActor* InActor, const FPCGWorldPartitionBuilderArgs& InArgs, FPCGCrc& OutCrc);
	static void LogRevertedActors(const TSet<TSoftObjectPtr<AActor>>& InRevertedActors);

	static void GetModifiedAndNewActorsFromDirtyPackages(const TArray<UPackage*>& InDirtyPackages, TArray<AActor*>& OutModifiedActors, TArray<AActor*>& OutNewActors);
	static void LogModifiedAndNewActorsFromDirtyPackages(const TArray<UPackage*>& InDirtyPackages);

	static void SetChainedDispatchToLocalComponents(UWorld* InWorld, bool bInChainedDispatch);

	static void UpdateEditingMode(UPCGComponent* InComponent, bool bForce = false);
private:
...
bool FPCGWorldPartitionBuilderArgs::InitializeFrom(const FPCGWorldPartitionCommandlineArgs& CommandlineArgs, FPCGWorldPartitionBuilderArgs& OutArgs)
{
	...
	if (SettingsPath.IsValid() || SettingsPath.IsPending())
	{
		if (UPCGBuilderSettings* BuilderSettings = Cast<UPCGBuilderSettings>(SettingsPath.LoadSynchronous()))
		{
			...
			BuilderArgs.bOneComponentAtATime = BuilderSettings->bOneComponentAtATime || BuilderSettings->bRevertUnchangedActors;
			...
			BuilderArgs.bRevertUnchangedActors = BuilderSettings->bRevertUnchangedActors;
			BuilderArgs.OutputsToIgnoreForRevert = BuilderSettings->OutputsToIgnoreForRevert;
	...
	// Log Revert parameters
	UE_LOG(LogPCGWorldPartitionBuilder, Display, TEXT("bRevertUnchangedActors '%d' (%s)"),
		BuilderArgs.bRevertUnchangedActors ? 1 : 0,
		FromDefault);

	UE_LOG(LogPCGWorldPartitionBuilder, Display, TEXT("OutputsToIgnoreForRevert '%s' (%s)"),
		BuilderArgs.OutputsToIgnoreForRevert.IsEmpty() ? TEXT("None") : *FString::Join(BuilderArgs.OutputsToIgnoreForRevert, TEXT(",")),
		FromDefault);
...
bool UPCGWorldPartitionBuilder::PreRun(UWorld* World, FPackageSourceControlHelper& PackageHelper)
{
	...
	FPCGWorldPartitionBuilder::SetChainedDispatchToLocalComponents(World, Args.bRevertUnchangedActors);

	RevertableActors.Empty();
	RevertedActors.Empty();
...
bool UPCGWorldPartitionBuilder::RunEntireWorld(UWorld* World, FPackageSourceControlHelper& PackageHelper)
{
	...
	// Before we run any generation, gather the revertable actors from the components we want to generate and the loaded pcg partition actors.
	// This will give us the pre generation Actorr to CRC mapping to compare against when generation completes.
	FPCGWorldPartitionBuilder::GetRevertableActorsFromComponents(ComponentsToGenerate, Args, RevertableActors);
	FPCGWorldPartitionBuilder::GetRevertablePartitionActors(World, Args, RevertableActors);
	...
	if (bGeneratedAnyComponent)
	{
		...
		DirtyPackages = FPCGWorldPartitionBuilder::GetDirtyPackages(World, AllowedAssetPackages);
	}
	
	// 5. Revert unchanged actors
	FPCGWorldPartitionBuilder::RevertUnchangedActors(DirtyPackages, Args, RevertableActors, RevertedActors);
...
bool UPCGWorldPartitionBuilder::SaveEntireWorldPackages(UWorld* World, FPackageSourceControlHelper& PackageHelper, TArray<UPackage*> DirtyPackages, TSet<FString> DeletedActorPackages)
{
	...
	for (int i = DirtyPackages.Num() - 1; i >= 0; --i)
	{
		if (UPackage* DirtyPackage = DirtyPackages[i])
		{
			...
			else
			{
				...
				if (DirtyPackage->HasAnyPackageFlags(PKG_NewlyCreated))
				{
					++NumNewPackages;
				}
				else
	...
	// Log New/Modified Actors
	FPCGWorldPartitionBuilder::LogModifiedAndNewActorsFromDirtyPackages(DirtyPackages);

	// Set final counts
	NumDeletedPackages = DeletedActorPackages.Num() + PackagesToDelete.Num();
...
bool UPCGWorldPartitionBuilder::RunPerCell(UWorld* InWorld, const FCellInfo& InCellInfo, FPackageSourceControlHelper& PackageHelper)
{
	...
	// Gather already loaded revertable Partition actors (before any modification occurs)
	FPCGWorldPartitionBuilder::GetRevertablePartitionActors(InWorld, Args, RevertableActors);
	...
	// Collect Components to Generate a first time, this will allow us to call FPCGWorldPartitionBuilder::UpdatePartitionedActors on ParttionActors with all their overlaps
	FPCGWorldPartitionBuilder::CollectComponentsToGenerate(InWorld, Args, GenerateComponentFilter, ComponentsToGenerate);

	// Gather revertable actors from the components to generate (this should be non-partitioned components as we already called GetRevertablePartitionActors at the beginning of this function)
	FPCGWorldPartitionBuilder::GetRevertableActorsFromComponents(ComponentsToGenerate, Args, RevertableActors);
	...
	// Gather newly loaded Partition actors before they get updated
	FPCGWorldPartitionBuilder::GetRevertablePartitionActors(InWorld, Args, RevertableActors);
...
{
	FPCGWorldPartitionBuilder::LogRevertedActors(RevertedActors);

	// Log final changes after combining packages (there may have been duplicates in the two deleted package lists).
	UE_LOG(LogPCGWorldPartitionBuilder, Display, TEXT("PostRun: Final package changes: %d new, %d modified, %d deleted, %d unchanged."), NumNewPackages, NumModifiedPackages , NumDeletedPackages, RevertedActors.Num());
...
{
	TArray<UPackage*> DirtyPackages = FPCGWorldPartitionBuilder::GetDirtyPackages(InWorld, AllowedAssetPackages);
	TArray<UPackage*> PackagesToDelete;
	...
	// Update stats
	NumNewPackages += CellNewPackageCount;
	NumModifiedPackages += CellModifiedPackageCount;
	NumDeletedPackages += CellDeletedPackageCount;
	...
	UE_LOG(LogPCGWorldPartitionBuilder, Display, TEXT("Cell package changes: %d modified, %d new, %d deleted."), CellNewPackageCount, CellModifiedPackageCount, CellDeletedPackageCount);
	return true;
}
	...
	bool& bOutGenerationErrors)
{
...

	{
		...
		Algo::TransformIf(Components, ValidComponents,
			[](const TWeakObjectPtr<UPCGComponent>& InWeakComponent)
			{
				return InWeakComponent.IsValid();
			},
			[](const TWeakObjectPtr<UPCGComponent>& InWeakComponent)
			{
				return InWeakComponent.Get();
			});

		ValidComponents.Sort([](const UPCGComponent& ComponentA, const UPCGComponent& ComponentB)
		{
			return ComponentA.GetPathName() < ComponentB.GetPathName();
		});

		for (UPCGComponent* Component : ValidComponents)
		{
...
FPCGTaskId FPCGWorldPartitionBuilder::GenerateComponent(UPCGComponent* InComponent, UWorld* InWorld, const FPCGWorldPartitionBuilderArgs& InArgs, FPCGTaskId InTaskDependency)
{
	...
	// @todo_pcg: revisit for 5.8:
	// Force update here to make sure ChangeTransientState gets called even if the editing mode might have been updated by previous AddGraphInstance calls.
	FPCGWorldPartitionBuilder::UpdateEditingMode(InComponent, /*bForce=*/true);
...
bool FPCGWorldPartitionBuilder::IsActorRevertable(AActor* InActor, const FPCGWorldPartitionBuilderArgs& InArgs, FPCGCrc& OutCrc)
{
	if (!InArgs.bRevertUnchangedActors)
	{
		return false;
	}
	...
	UE_LOG(LogPCGWorldPartitionBuilder, VeryVerbose, TEXT("IsActorRevertable : '%s' : begin"), *InActor->GetActorLabel());

	const bool bLocalComponents = InActor->IsA<APCGPartitionActor>();
	bool bCanRevertActor = true;
	TArray<const UPCGComponent*> PCGComponents;
	InActor->ForEachComponent(/*bIncludeFromChildActors=*/false, [bLocalComponents, &bCanRevertActor, &PCGComponents](const UActorComponent* InComponent)
	{
		const UPCGComponent* PCGComponent = Cast<UPCGComponent>(InComponent);
		if (PCGComponent && (PCGComponent->IsLocalComponent() == bLocalComponents))
		{
			PCGComponents.Add(PCGComponent);
		}
		else if (!InComponent->IsDefaultSubobject())
		{
			bCanRevertActor = false;
			UE_LOG(LogPCGWorldPartitionBuilder, VeryVerbose, TEXT("IsActorRevertable : '%s' : Can't revert actor because of non default component '%s'"), *InComponent->GetOwner()->GetActorLabel(), *InComponent->GetName());
		}
	});

	if (!bCanRevertActor)
	{
		UE_LOG(LogPCGWorldPartitionBuilder, VeryVerbose, TEXT("IsActorRevertable : '%s' : end"), *InActor->GetActorLabel());
		return false;
	}
	...
	{
		FArchiveCrc32 Ar;
		bool bComponentGenerated = PCGComponent->bGenerated;
		Ar << bComponentGenerated;
		if (bComponentGenerated)
		{
			FBox ComponentLastGeneratedBounds = PCGComponent->LastGeneratedBounds;
			Ar << ComponentLastGeneratedBounds;
		}

		Crc.Combine(Ar.GetCrc());

		PCGComponent->ForEachConstManagedResource([&Crc, &bCanRevertActor, InActor](const UPCGManagedResource* ManagedResource)
		{
			FPCGCrc ResourceCrc = ManagedResource->GetCrc();
			if (!ResourceCrc.IsValid())
			{
				bCanRevertActor = false;
				UE_LOG(LogPCGWorldPartitionBuilder, VeryVerbose, TEXT("IsActorRevertable : '%s' Can't revert actor because of invalid crc for resource : '%s'"), *InActor->GetActorLabel(), *ManagedResource->GetName());
			}
			else
			{
				Crc.Combine(ManagedResource->GetCrc());
				UE_LOG(LogPCGWorldPartitionBuilder, VeryVerbose, TEXT("IsActorRevertable : '%s' : resource '%s' : crc '%d'"), *InActor->GetActorLabel(), *ManagedResource->GetName(), ResourceCrc.GetValue());
			}
		});

		for (const FPCGTaggedData& Output : PCGComponent->GetGeneratedGraphOutput().TaggedData)
		{
			if (InArgs.OutputsToIgnoreForRevert.Contains(Output.Pin.ToString()))
			{
				UE_LOG(LogPCGWorldPartitionBuilder, VeryVerbose, TEXT("IsActorRevertable : '%s' : output pin ignored: '%s'"), *InActor->GetActorLabel(), *Output.Pin.ToString());
				continue;
			}

			FPCGCrc OutputCrc = Output.ComputeCrc(/*bFullDataCrc=*/true);
			UE_LOG(LogPCGWorldPartitionBuilder, VeryVerbose, TEXT("IsActorRevertable : '%s' : output pin: '%s' : crc '%d'"), *InActor->GetActorLabel() , *Output.Pin.ToString(), OutputCrc.GetValue());
			Crc.Combine(OutputCrc);
		}
	}
	...
	OutCrc = Crc;
	UE_LOG(LogPCGWorldPartitionBuilder, VeryVerbose, TEXT("IsActorRevertable : '%s' : crc : '%d' : end"), *InActor->GetActorLabel(), OutCrc.GetValue());
	return true;
}

void FPCGWorldPartitionBuilder::GetRevertableActorsFromComponents(TArray<TWeakObjectPtr<UPCGComponent>> InComponentsToGenerate, const FPCGWorldPartitionBuilderArgs& InArgs, TMap<TSoftObjectPtr<AActor>, FPCGCrc>& OutRevertableActors)
{
	for (TWeakObjectPtr<UPCGComponent> Component : InComponentsToGenerate)
	{
		AActor* Owner = Component.IsValid() ? Component->GetOwner() : nullptr;
		if (Owner)
		{
			FPCGCrc Crc;
			if (!OutRevertableActors.Contains(Owner) && FPCGWorldPartitionBuilder::IsActorRevertable(Owner, InArgs, Crc))
			{
				OutRevertableActors.Add(Owner, Crc);
			}
		}
	}
}

void FPCGWorldPartitionBuilder::GetRevertablePartitionActors(UWorld* InWorld, const FPCGWorldPartitionBuilderArgs& InArgs, TMap<TSoftObjectPtr<AActor>, FPCGCrc>& OutRevertableActors)
{
	for (TActorIterator<APCGPartitionActor> It(InWorld); It; ++It)
	{
		FPCGCrc Crc;
		if (!OutRevertableActors.Contains(*It) && FPCGWorldPartitionBuilder::IsActorRevertable(*It, InArgs, Crc))
		{
			OutRevertableActors.Add(*It, Crc);
		}
	}
}

void FPCGWorldPartitionBuilder::GetRevertableActorsFromDirtyPackages(TArray<UPackage*> InDirtyPackages, const FPCGWorldPartitionBuilderArgs& InArgs, TMap<TSoftObjectPtr<AActor>, FPCGCrc>& OutRevertableActors)
{
	for (UPackage* DirtyPackage : InDirtyPackages)
	{
		if (AActor* Actor = AActor::FindActorInPackage(DirtyPackage))
		{
			FPCGCrc Crc;
			if (!OutRevertableActors.Contains(Actor) && FPCGWorldPartitionBuilder::IsActorRevertable(Actor, InArgs, Crc))
			{
				OutRevertableActors.Add(Actor, Crc);
			}
		}
	}
}

void FPCGWorldPartitionBuilder::RevertUnchangedActors(TArray<UPackage*>& InOutDirtyPackages, const FPCGWorldPartitionBuilderArgs& InArgs, const TMap<TSoftObjectPtr<AActor>, FPCGCrc>& InRevertableActors, TSet<TSoftObjectPtr<AActor>>& InOutRevertedActors)
{
	TMap<TSoftObjectPtr<AActor>, FPCGCrc> DirtyRevertableActors;
	FPCGWorldPartitionBuilder::GetRevertableActorsFromDirtyPackages(InOutDirtyPackages, InArgs, DirtyRevertableActors);

	for (const auto& KVP : DirtyRevertableActors)
	{
		// Actor found in revertable actors and crc matches
		if (const FPCGCrc* PreCrc = InRevertableActors.Find(KVP.Key); PreCrc && *PreCrc == KVP.Value)
		{
			if (InOutDirtyPackages.Remove(KVP.Key->GetPackage()))
			{
				InOutRevertedActors.Add(KVP.Key);
			}
		} 
		else
		{
			// actor not found and or crc doesn't match. This can happen if actor is processed more than once in a per cell situation, one cell might not change the actor but the next one does.
			InOutRevertedActors.Remove(KVP.Key);
		}
	}
}

void FPCGWorldPartitionBuilder::GetModifiedAndNewActorsFromDirtyPackages(const TArray<UPackage*>& InDirtyPackages, TArray<AActor*>& OutModifiedActors, TArray<AActor*>& OutNewActors)
{
	for (UPackage* DirtyPackage : InDirtyPackages)
	{
		if (AActor* Actor = AActor::FindActorInPackage(DirtyPackage))
		{
			...
			{
				OutModifiedActors.Add(Actor);
			}
		}
	}
}

void FPCGWorldPartitionBuilder::LogRevertedActors(const TSet<TSoftObjectPtr<AActor>>& InRevertedActors)
{
	// Log actor changes
	for (TSoftObjectPtr<AActor> RevertedActorPtr : InRevertedActors)
	{
		if (AActor* RevertedActor = RevertedActorPtr.Get())
		{
			UE_LOG(LogPCGWorldPartitionBuilder, Display, TEXT("Unchanged actor '%s' (name '%s' - package '%s')"), *RevertedActor->GetActorLabel(), *RevertedActor->GetName(), *RevertedActor->GetPackage()->GetName());
		}
	}
}

void FPCGWorldPartitionBuilder::LogModifiedAndNewActorsFromDirtyPackages(const TArray<UPackage*>& InDirtyPackages)
{
	TArray<AActor*> ModifiedActors;
	TArray<AActor*> NewActors;
	FPCGWorldPartitionBuilder::GetModifiedAndNewActorsFromDirtyPackages(InDirtyPackages, ModifiedActors, NewActors);

	// Log actor changes
	for (AActor* ModifiedActor : ModifiedActors)
	{
		UE_LOG(LogPCGWorldPartitionBuilder, Display, TEXT("Modified actor '%s' (name '%s' - package '%s')"), *ModifiedActor->GetActorLabel(), *ModifiedActor->GetName(), *ModifiedActor->GetPackage()->GetName());
	}

	for (AActor* NewActor : NewActors)
	{
		UE_LOG(LogPCGWorldPartitionBuilder, Display, TEXT("Added actor '%s' (name '%s' - package '%s')"), *NewActor->GetActorLabel(), *NewActor->GetName(), *NewActor->GetPackage()->GetName());
	}
}

void FPCGWorldPartitionBuilder::SetChainedDispatchToLocalComponents(UWorld* InWorld, bool bInChainedDispatch)
{
	UPCGSubsystem* PCGSubsystem = UWorld::GetSubsystem<UPCGSubsystem>(InWorld);
	check(PCGSubsystem);
	PCGSubsystem->SetChainedDispatchToLocalComponents(bInChainedDispatch);
}

void FPCGWorldPartitionBuilder::UpdateEditingMode(UPCGComponent* InComponent, bool bForce)
{
	if (InComponent->GetSerializedEditingMode() == EPCGEditorDirtyMode::LoadAsPreview && (bForce || InComponent->GetEditingMode() != EPCGEditorDirtyMode::LoadAsPreview))
	{
...
TArray<UPackage*> FPCGWorldPartitionBuilder::GetDirtyPackages(const UWorld* InWorld, const TSet<FString>& AllowedAssetPackages)
{
	TArray<UPackage*> DirtyPackages;
	TSet<UPackage*> ExternalPackages;

	const UPackage* WorldPackage = InWorld->GetPackage();
	const bool bIsPartitioned = InWorld->IsPartitionedWorld();

	ExternalPackages.Append(WorldPackage->GetExternalPackages());

	FEditorFileUtils::GetDirtyWorldPackages(DirtyPackages, /*ShouldIgnorePackageFunction=*/[bIsPartitioned, WorldPackage, &AllowedAssetPackages, &ExternalPackages](UPackage* DirtyPackage)
	{
		const bool bExplicitelyAllowedAsset = AllowedAssetPackages.Contains(DirtyPackage->GetName());

		// Don't allow modification of World Partition umap unless allowed in AllowedAssetPackages
		if (WorldPackage == DirtyPackage)
		{
			return bIsPartitioned ? !bExplicitelyAllowedAsset : false;
		}
		
		if (bIsPartitioned)
		{
			// WP world should only allow current world external packages (actors/folders) or explicitely allowed assets to be dirtied.
			return !bExplicitelyAllowedAsset && !ExternalPackages.Contains(DirtyPackage);
		}
		else
		{
			// Non-WP world should only allow umap to be dirtied and explicitely allowed assets.
			return !bExplicitelyAllowedAsset;
		}
	});
	...
	if (!AllowedAssetPackages.IsEmpty())
	{
		...
		FEditorFileUtils::GetDirtyContentPackages(DirtyAssetPackages, /*ShouldIgnorePackageFunction=*/[&AllowedAssetPackages](UPackage* DirtyPackage)
		{
			return !AllowedAssetPackages.Contains(DirtyPackage->GetName());
		});

		DirtyPackages.Append(DirtyAssetPackages);
	}
...
bool FPCGWorldPartitionBuilder::UpdatePartitionedActors(UWorld* InWorld, const TMap<TSoftObjectPtr<UPCGComponent>, FWorldPartitionReference>& InPartitionedComponents, TSet<FString>& InOutDeletedActorPackages)
{
	...
	for (TActorIterator<APCGPartitionActor> It(InWorld); It; ++It)
	{
		...
		if (PartitionActor->GetLevel() == InWorld->PersistentLevel)
		{
			...
			if (PartitionActor->CleanupDeadGraphInstancesInternal(/*bCleanupTest=*/true, &DeadLocalComponents))
			{
				...
				PartitionActor->CleanupDeadGraphInstancesInternal(/*bCleanupTest=*/false);
			}

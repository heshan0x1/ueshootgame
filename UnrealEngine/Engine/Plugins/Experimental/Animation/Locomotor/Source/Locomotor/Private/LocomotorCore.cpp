...
{
	bPostInitialized = false;
	float WrappedPhaseOffset = FMath::Fmod(FMath::Fmod(PhaseOffset, 1.0f) + 1.0f, 1.0f);
	return FootSets.Emplace(FLocomotorFootSet(WrappedPhaseOffset, FootSets.Num()));
}
	...
	const FLocomotorFootSettings& InSettings)
{
	...
	// update cached pointers to all feet
	// NOTE: we reset and update all pointers here in case the foot arrays inside the sets
	// were reallocated which would invalidate all the pointers in the cache.
	AllFeet.Reset(0);
	for (FLocomotorFootSet& FootSet : FootSets)
	{
		for (FLocomotorFoot& Foot : FootSet.GetFeet())
		{
			AllFeet.Add(&Foot);
		}
	}
...
void FLocomotor::UpdateFeetTargets()
{
	...
	for (FLocomotorFoot* FootPtr : AllFeet)
	{
		...
		{
			const int32 FootSetIndex = FMath::Clamp(Foot.Settings.FootSetIndex, 0, FootSets.Num() - 1);
			const float PhaseOffsetOfSet = FootSets[FootSetIndex].GetPhaseOffset();

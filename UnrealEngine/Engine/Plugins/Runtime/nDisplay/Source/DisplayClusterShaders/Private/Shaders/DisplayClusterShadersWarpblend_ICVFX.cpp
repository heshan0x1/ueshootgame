...
public:
	...
	{
		// Only compile for desktop-class "PC" shader platforms (filters out mobile and console).
...
public:
	...
	EDisplayClusterShaderParametersICVFX_ChromakeySource GetCameraOverlapParameters(FIcvfxRenderPassData& RenderPassData)
	{
		...
		EDisplayClusterShaderParametersICVFX_ChromakeySource OutChromakeySource = EDisplayClusterShaderParametersICVFX_ChromakeySource::Default;
	...
	bool GetCameraChromakeyParameters(FIcvfxRenderPassData& RenderPassData, const EDisplayClusterShaderParametersICVFX_ChromakeySource InChromakeySource)
	{
		...
		EDisplayClusterShaderParametersICVFX_ChromakeySource ChromakeySource = Camera.ChromakeySource;

		// Override by overlap
		if (InChromakeySource != EDisplayClusterShaderParametersICVFX_ChromakeySource::Default)
		{
			ChromakeySource = InChromakeySource;
		};


		switch (ChromakeySource)
		{
		case EDisplayClusterShaderParametersICVFX_ChromakeySource::ChromakeyLayers:
		{
			...
			if (RenderPassData.PSParameters.InnerCameraTexture == nullptr || Camera.Chromakey.Texture == nullptr)
			{
				// There is no InCamera texture and no frame colour, we have nothing to render.
				// Disable this camera, wrong combination
				RenderPassData.PSPermutationVector.Set<IcvfxShaderPermutation::FIcvfxShaderInnerCamera>(false);
		...
		case EDisplayClusterShaderParametersICVFX_ChromakeySource::Default:
			// By default chromakey is disabled
		...
		// Protection against invalid parameter combinations
		if (RenderPassData.PSParameters.InnerCameraTexture == nullptr)
		{
			// Disable this camera, wrong combination
			RenderPassData.PSPermutationVector.Set<IcvfxShaderPermutation::FIcvfxShaderInnerCamera>(false);
	...
	void InitRenderPass(FIcvfxRenderPassData& RenderPassData)
	{
		...
		if (WarpBlendParameters.WarpInterface.IsValid())
		{
			...
			{
				// Both LightCards textures use linear gamma, so they have the same gamma correction at the end.
				{
					// The LightCard texture is in linear color space and should be converted to the target gamma.
					// Math: GammaDst = Pow( LinearSrc, (1/Gamma) ).
					if (ICVFXParameters.LightCardGamma > KINDA_SMALL_NUMBER)
					{
						RenderPassData.PSParameters.LightCardGamma = 1.0f / ICVFXParameters.LightCardGamma;
					}
					else
					{
						RenderPassData.PSParameters.LightCardGamma = 1.0f;
					}
				}
			...
			{
				// Overlap is first priority
				EDisplayClusterShaderParametersICVFX_ChromakeySource ChromakeySourceOverride = GetCameraOverlapParameters(RenderPassData);

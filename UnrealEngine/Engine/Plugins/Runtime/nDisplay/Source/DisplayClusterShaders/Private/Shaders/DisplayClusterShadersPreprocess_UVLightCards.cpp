...
{
	/**
	* Returns the render scene associated with the given view.
	* May return nullptr if the view or its family/scene is invalid.
	*/
	static const FScene* GetRenderSceneFromView(const FSceneView* InView)
	{
		if (InView && InView->Family && InView->Family->Scene)
		{
			return InView->Family->Scene->GetRenderScene();
		}
...
public:
	FUVLightCardPassProcessor(const FSceneView* InView, FMeshPassDrawListContext* InDrawListContext)
		: FMeshPassProcessor(
			EMeshPass::Num, 
			GetRenderSceneFromView(InView),
			InView ? InView->GetFeatureLevel() : GMaxRHIFeatureLevel,
			InView,
			InDrawListContext)
	{ }
	...
	virtual void AddMeshBatch(const FMeshBatch& RESTRICT MeshBatch, uint64 BatchElementMask, const FPrimitiveSceneProxy* RESTRICT PrimitiveSceneProxy, int32 StaticMeshId = -1) override final
	{
		...
		FMeshPassProcessorRenderState DrawRenderState;
		{
			...
			// Substrate materials always use premultiplied alpha for translucent rendering.
			// Apply a custom BlendState so SrcColor is not multiplied by alpha again.
			if (bIsTranslucent && Material.IsSubstrateMaterial())
			{
				// Substrate translucent blending:
				// Output = SrcColor + DstColor * (1 - SrcAlpha)
			...
			else
			{
				...
				// Output = SrcColor * SrcAlpha + DstColor * (1 - SrcAlpha)
				DrawRenderState.SetBlendState(
					TStaticBlendState<CW_RGBA,

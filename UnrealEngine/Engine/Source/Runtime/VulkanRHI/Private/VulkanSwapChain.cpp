...
class FVulkanCommandsHelper : public FSelfRegisteringExec
{
	virtual bool Exec_Dev(class UWorld* InWorld, const TCHAR* Cmd, FOutputDevice& Ar)
	{
		...
		{
			GSimulateLostSurfaceInNextPresent = true;
			Ar.Log(FString::Printf(TEXT("Vulkan: simulating lost surface next frame's present")));
			return true;
		}
		...
		else if (FParse::Command(&Cmd, TEXT("VULKAN_SIMULATE_SWAPCHAIN_LOST_SURFACE")))
		{
			GSimulateLostSurfaceInNextSwapChain = true;
			Ar.Log(FString::Printf(TEXT("Vulkan: simulating lost surface on next swapchain creation")));
			return true;
		}
		else
		{
...
{
	if (GSimulateLostSurfaceInNextPresent)
	{
		GSimulateLostSurfaceInNextPresent = false;
...
#define VERIFYVULKANSURFACERESULT(VkFunction) \
	{\
		...
		if (ScopedResult == VK_ERROR_SURFACE_LOST_KHR) \
		{ \
			DestroyRecreateInfo(); \
			VulkanRHI::vkDestroySurfaceKHR(InInstance, Surface, VULKAN_CPU_ALLOCATOR); \
			return nullptr; \
		} \
	...
	auto DestroyRecreateInfo = [RecreateInfo, InInstance, &Device]()
	{
		if (RecreateInfo != nullptr)
		{
			...
			{
				VulkanRHI::vkDestroySurfaceKHR(InInstance, RecreateInfo->Surface, VULKAN_CPU_ALLOCATOR);
				RecreateInfo->Surface = VK_NULL_HANDLE;
			}
		}
	};
...
#if !UE_BUILD_SHIPPING
	if (GSimulateLostSurfaceInNextSwapChain)
	{
		GSimulateLostSurfaceInNextSwapChain = false;
		VERIFYVULKANSURFACERESULT(VK_ERROR_SURFACE_LOST_KHR);
	}
#endif

	uint32 NumFormats;
	VERIFYVULKANSURFACERESULT(VulkanRHI::vkGetPhysicalDeviceSurfaceFormatsKHR(Device.GetPhysicalHandle(), Surface, &NumFormats, nullptr));
	if (NumFormats == 0)
	{
		UE_LOG(LogVulkanRHI, Warning, TEXT("vkGetPhysicalDeviceSurfaceFormatsKHR returned 0 formats! Handling as surface lost."));
		VERIFYVULKANSURFACERESULT(VK_ERROR_SURFACE_LOST_KHR);
	}
	else if (InOutDesiredNumBackBuffers && (NumFormats < *InOutDesiredNumBackBuffers))
	{
		// UE_LOG(LogVulkanRHI, Warning, TEXT("vkGetPhysicalDeviceSurfaceFormatsKHR returned %u formats, expected %u!"), NumFormats, *InOutDesiredNumBackBuffers);
	}

	TArray<VkSurfaceFormatKHR> Formats;
	Formats.AddZeroed(NumFormats);
	VERIFYVULKANSURFACERESULT(VulkanRHI::vkGetPhysicalDeviceSurfaceFormatsKHR(Device.GetPhysicalHandle(), Surface, &NumFormats, Formats.GetData()));
	...
	if (FVulkanPlatform::SupportsQuerySurfaceProperties())
	{
		...
		VERIFYVULKANSURFACERESULT(VulkanRHI::vkGetPhysicalDeviceSurfacePresentModesKHR(Device.GetPhysicalHandle(), Surface, &NumFoundPresentModes, nullptr));
		check(NumFoundPresentModes > 0);

		TArray<VkPresentModeKHR> FoundPresentModes;
		FoundPresentModes.AddZeroed(NumFoundPresentModes);
		VERIFYVULKANSURFACERESULT(VulkanRHI::vkGetPhysicalDeviceSurfacePresentModesKHR(Device.GetPhysicalHandle(), Surface, &NumFoundPresentModes, FoundPresentModes.GetData()));
	...
	VERIFYVULKANSURFACERESULT(VulkanRHI::vkGetPhysicalDeviceSurfaceCapabilitiesKHR(Device.GetPhysicalHandle(), Surface, &SurfProperties));

	VkBool32 bSupportsPresent;
	VERIFYVULKANSURFACERESULT(VulkanRHI::vkGetPhysicalDeviceSurfaceSupportKHR(Device.GetPhysicalHandle(), Device.GetPresentQueue()->GetFamilyIndex(), Surface, &bSupportsPresent));
	ensure(bSupportsPresent);

	const VkSurfaceTransformFlagBitsKHR PreTransform = IsVulkanPreTransformEnabled(GMaxRHIShaderPlatform) ? SurfProperties.currentTransform : VK_SURFACE_TRANSFORM_IDENTITY_BIT_KHR;
	...
	const uint32 DesiredNumBuffers = SurfProperties.maxImageCount > 0 ? FMath::Clamp(*InOutDesiredNumBackBuffers, SurfProperties.minImageCount, SurfProperties.maxImageCount) : *InOutDesiredNumBackBuffers;

	const uint32 SizeX = FVulkanPlatform::SupportsQuerySurfaceProperties() ? (SurfProperties.currentExtent.width == 0xFFFFFFFF ? Width : SurfProperties.currentExtent.width) : Width;
	const uint32 SizeY = FVulkanPlatform::SupportsQuerySurfaceProperties() ? (SurfProperties.currentExtent.height == 0xFFFFFFFF ? Height : SurfProperties.currentExtent.height) : Height;
	...
	VkResult Result = FVulkanPlatform::CreateSwapchainKHR(WindowContext, Device.GetPhysicalHandle(), Device.GetHandle(), &SwapChainInfo, VULKAN_CPU_ALLOCATOR, &SwapChain);
	...
	if (Device.GetOptionalExtensions().HasEXTFullscreenExclusive && Result == VK_ERROR_INITIALIZATION_FAILED)
	{
		...
		Result = FVulkanPlatform::CreateSwapchainKHR(WindowContext, Device.GetPhysicalHandle(), Device.GetHandle(), &SwapChainInfo, VULKAN_CPU_ALLOCATOR, &SwapChain);
	}
#endif
	VERIFYVULKANSURFACERESULT(Result);

	Width = FMath::Min(Width, SwapChainInfo.imageExtent.width);
	Height = FMath::Min(Height, SwapChainInfo.imageExtent.height);

	uint32 NumSwapChainImages;
	VERIFYVULKANRESULT_EXPANDED(VulkanRHI::vkGetSwapchainImagesKHR(Device.GetHandle(), SwapChain, &NumSwapChainImages, nullptr));
	...
	VERIFYVULKANRESULT_EXPANDED(VulkanRHI::vkGetSwapchainImagesKHR(Device.GetHandle(), SwapChain, &NumSwapChainImages, OutImages.GetData()));

	*InOutDesiredNumBackBuffers = NumSwapChainImages;

	return new FVulkanSwapChain(InInstance, Device, Surface, SwapChain, PreTransform, Width, Height, bIsFullScreen, NumSwapChainImages, LockToVsync, WindowContext);
#undef VERIFYVULKANSURFACERESULT
}

FVulkanSwapChain::FVulkanSwapChain(
	VkInstance InInstance, FVulkanDevice& InDevice, VkSurfaceKHR InSurface,	VkSwapchainKHR InSwapChain, 
	VkSurfaceTransformFlagBitsKHR InPreTransform, uint32 InWidth, uint32 InHeight, bool InIsFullscreen,
	uint32 NumSwapChainImages, int8 InLockToVsync, FVulkanPlatformWindowContext& WindowContext)
	: Instance(InInstance)
	, Device(InDevice)
	, Surface(InSurface)
	, SwapChain(InSwapChain)
	, PreTransform(InPreTransform)
	, InternalWidth(InWidth)
	, InternalHeight(InHeight)
	, bInternalFullScreen(InIsFullscreen)
	...
	, LockToVsync(InLockToVsync)
{
	...
	FVulkanFenceManager& FenceMgr = Device.GetFenceManager();
	for (uint32 BufferIndex = 0; BufferIndex < NumSwapChainImages; ++BufferIndex)
	{
		ImageAcquiredFences[BufferIndex] = FenceMgr.AllocateFence(true);
	}
#endif
	ImageAcquiredSemaphore.SetNum(NumSwapChainImages);
	for (uint32 BufferIndex = 0; BufferIndex < NumSwapChainImages; ++BufferIndex)
	{
		ImageAcquiredSemaphore[BufferIndex] = new FVulkanSemaphore(Device, EVulkanSemaphoreFlags::ExternallySignaled);
	}
}
...
void FVulkanSwapChain::Destroy(FVulkanSwapChainRecreateInfo* RecreateInfo)
{
	...
	const bool bRecreate = RecreateInfo && GVulkanKeepSwapChain;
	if (bRecreate)
	{
	...
	{
		FVulkanPlatform::DestroySwapchainKHR(Device.GetHandle(), SwapChain, VULKAN_CPU_ALLOCATOR);
	}
	...
	ImageAcquiredFences.Empty();
#endif

	ImageAcquiredSemaphore.Empty();
...
int32 FVulkanSwapChain::AcquireImageIndex(FVulkanSemaphore** OutSemaphore)
{
	...
	*OutSemaphore = ImageAcquiredSemaphore[SemaphoreIndex];
...
#if VULKAN_USE_IMAGE_ACQUIRE_FENCES
	{
		...
		const bool bResult = FenceMgr.WaitForFence(ImageAcquiredFences[SemaphoreIndex], UINT64_MAX);
		ensure(bResult);
	}
#endif
...
FVulkanSwapChain::EStatus FVulkanSwapChain::Present(FVulkanQueue* PresentQueue, FVulkanSemaphore* BackBufferRenderingDoneSemaphore)
{
	check(CurrentImageIndex != -1);

	if (Device.UseMinimalSubmits())
	{
		GVulkanRHI->KickSubmissionThread(true);
	}
...

	{
		...
		SCOPED_NAMED_EVENT(VulkanQueuePresent, FColor::Yellow);

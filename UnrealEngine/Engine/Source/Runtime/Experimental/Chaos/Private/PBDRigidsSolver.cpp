...
namespace Chaos
{
	namespace CVars
	{
		...
		bool bSingleThreadPushData = true;
...
namespace Chaos
{
		...
		void DoWork()
		{
...

			{
				...
				while (StepsRemaining > 0 && TimeRemaining > MinDeltaTime)
				{
					...
					// we only want to process once per step because we don't want to apply the same forces multiple time during this solver tick
			...
			if(MDeltaTime > 0)
			{
				CSV_SCOPED_TIMING_STAT(PhysicsVerbose, StepSolver_PullData);
	...
	FPBDRigidsSolver::FPBDRigidsSolver(const EMultiBufferMode BufferingModeIn, UObject* InOwner, FReal InAsyncDt)
		...
		, TaskDispatcher(MakeUnique<Private::FTaskDispatcherSolver>())
		, QueryMaterials_External()
		, QueryMaterialMasks_External()
		, SimMaterials()
		, SimMaterialMasks()
		, UniqueIndices()
	...
	{
		bool bHasIntegrationCompleted = false;
		for (ISimCallbackObject* Callback : SimCallbackObjects)
		{
			if (Callback->HasOption(ESimCallbackOptions::PostIntegrate))
			{
				if (!bHasIntegrationCompleted)
				{
					// If a post integrate callback is called, 
					// we need to wait for all workers threads integrating to finish.
					MEvolution->WaitIntegrationComplete();
					bHasIntegrationCompleted = true;
				}
	...
	void FPBDRigidsSolver::ClearCollisionEventsForProxy(const IPhysicsProxyBase* Proxy)
	{
		// @todo(chaos): Race Condition: In async mode UnregisterObject may be called while the physics thread is
		// running, and perhaps just about to dispatch events. The command below will not execute until the 
		// next frame, so we may still get collision events referencing our owner (via the UserData). 
		// See FEventDefaults::RegisterCollisionEvent. The logic for collecting events checks Proxy->GetMarkDeleted(), 
		// but the events may have already been collected, and that flag is also not intended for PT access and is
		// not thread safe.
		// For now we require the user to check GetMarkDeleted() in their collision handler to avoid the race condition.
	...
	void FPBDRigidsSolver::UnregisterObject(FSingleParticlePhysicsProxy* Proxy)
	{
		...
		{
			// The collision manager is also modified from FPBDRigidsSolver::PushPhysicsState which uses the 
			// MarshallingManager lock so we need to lock this here as well.
			// @todo(chaos): should we lock this for the whole function?
			Chaos::Private::FChaosAsyncInitBodyWriteScopeLock MarshallingManagerLock(MarshallingManager.GetMarshallingManagerLock());
		...
		EnqueueCommandImmediate([Proxy, UniqueIdx, this]()
		{
			...
			// Remove stale events for particles that no longer exist
			// See comments in ClearCollisionEventsForProxy for potential race condition
			ClearCollisionEventsForProxy(Proxy);
	...
	void FPBDRigidsSolver::UnregisterObject(FGeometryCollectionPhysicsProxy* InProxy)
	{
		...
		EnqueueCommandImmediate([InProxy, this]()
		{
			// Remove stale events for particles that no longer exist
			// See comments in ClearCollisionEventsForProxy for potential race condition
			ClearCollisionEventsForProxy(InProxy);

			GeometryCollectionPhysicsProxies_Internal.RemoveSingle(InProxy);
			InProxy->SyncBeforeDestroy();

			const int32 OffsetForRewind = (bResimDelayDestroyObject && MRewindData) ? MRewindData->Capacity() : 0;
			PendingDestroyGeometryCollectionPhysicsProxy.Add(FPendingProxyDestroyInfo<FGeometryCollectionPhysicsProxy>{ InProxy, GetCurrentFrame() + OffsetForRewind });
		});
	}
	...
	void FPBDRigidsSolver::UnregisterObject(FClusterUnionPhysicsProxy* Proxy)
	{
		...
		EnqueueCommandImmediate(
			[Proxy, this]()
			{
				...
				const int32 OffsetForRewind = (bResimDelayDestroyObject && MRewindData) ? MRewindData->Capacity() : 0;
				PendingDestroyClusterUnionProxy.Add(FPendingProxyDestroyInfo<FClusterUnionPhysicsProxy>{ Proxy, GetCurrentFrame() + OffsetForRewind });
			}
		);
	}
	...
	void FPBDRigidsSolver::Reset()
	{
		...
		if (GetRewindData())
		{
			EnableRewindCapture(GetRewindData()->Capacity(), GetUseCollisionResimCache());
		}
	...
	void FPBDRigidsSolver::DestroyPendingProxies_Internal()
	{
		...
		for (int32 Idx = PendingDestroyClusterUnionProxy.Num() - 1; Idx >= 0; --Idx)
		{
			FPendingProxyDestroyInfo<FClusterUnionPhysicsProxy>& Info = PendingDestroyClusterUnionProxy[Idx];

			// Are we on or after the frame where we want to destroy the proxy
			// Note: GetCurrentFrame() - 1 because this happens at the end of the physics frame after frame number has been incremented already
			if (Info.DestroyOnStep <= (GetCurrentFrame() - 1) || IsShuttingDown()) 
			{
				if (Info.Proxy != nullptr)
				{
					...
					if (PhysicsObjectUnregistrationWatchers.Num() > 0)
					{
						...
						{
							Callback->OnPhysicsObjectUnregistered_Internal(Info.Proxy->GetPhysicsObjectHandle());
						}
					}
					...
					if (FPBDRigidsEvolutionGBF* Evolution = GetEvolution())
					{
						...
						Evolution->GetRigidClustering().GetClusterUnionManager().DestroyClusterUnion(Info.Proxy->GetClusterUnionIndex());
					}

					delete Info.Proxy;
				}

				PendingDestroyClusterUnionProxy.RemoveAtSwap(Idx, EAllowShrinking::No);
			}
		}
		...
		for (int32 Idx = PendingDestroyGeometryCollectionPhysicsProxy.Num() - 1; Idx >= 0; --Idx)
		{
			FPendingProxyDestroyInfo<FGeometryCollectionPhysicsProxy>& Info = PendingDestroyGeometryCollectionPhysicsProxy[Idx];

			GetEvolution()->GetRigidClustering().CleanupInternalClustersForProxies(MakeArrayView(reinterpret_cast<IPhysicsProxyBase**>(&Info.Proxy), 1));
			...
			if (Info.DestroyOnStep <= (GetCurrentFrame() - 1) || IsShuttingDown())
			{
				if (Info.Proxy != nullptr)
				{
					...
					if (PhysicsObjectUnregistrationWatchers.Num() > 0)
					{
						TArray<FPhysicsObjectHandle> PhysicsObjects = Info.Proxy->GetAllPhysicsObjects();
					...
					Info.Proxy->OnRemoveFromSolver(this);
					delete Info.Proxy;
				}

				PendingDestroyGeometryCollectionPhysicsProxy.RemoveAtSwap(Idx, EAllowShrinking::No);
			}
		}
	}
		...
		DirtyProxiesData->ForEachProxy([this, &ProcessProxyPT, Manager, &NumInitializedGCProxies](int32 DataIdx,FDirtyProxy& Dirty)
		{
			if(Dirty.Proxy->GetIgnoreDataOnStep_Internal() != CurrentFrame)
			{
				switch(Dirty.Proxy->GetType())
				{
					case EPhysicsProxyType::SingleParticleProxy:
					{
						FSingleParticlePhysicsProxy* Proxy = static_cast<FSingleParticlePhysicsProxy*>(Dirty.Proxy);
					...
					case EPhysicsProxyType::GeometryCollectionType:
					{
						FGeometryCollectionPhysicsProxy* Proxy = static_cast<FGeometryCollectionPhysicsProxy*>(Dirty.Proxy);
	...
	{
		return Callback->HasOption(ESimCallbackOptions::Presimulate) ||
			Callback->HasOption(ESimCallbackOptions::PreSolve) ||
			Callback->HasOption(ESimCallbackOptions::PostSolve) ||
			Callback->HasOption(ESimCallbackOptions::PreIntegrate) ||
			Callback->HasOption(ESimCallbackOptions::PostIntegrate);
	}
	...
	void FPBDRigidsSolver::ProcessPushedData_Internal(FPushPhysicsData& PushData)
	{
		...
		{
			if (HasSimulationCallback(SimCallbackObject))
			{
		...
		if (FRewindData* RewindData = GetRewindData())
		{
			RewindData->CacheCurrentDirtyData(FFrameAndPhase::PrePushData);
		}
		...
		// If we are not resimulating, insert scheduled commands for this step into the current PushData to be executed
		if (!IsResimming())
		{
			TArray<FSimCallbackCommandObject*>& ScheduledSimCommands = MarshallingManager.GetSimCommandsScheduled_Internal();
			for (int32 Idx = ScheduledSimCommands.Num() - 1; Idx >= 0; Idx--)
			{
				FSimCallbackCommandObject* SimCallbackObject = ScheduledSimCommands[Idx];
				if (SimCallbackObject->ShouldExecute(CurrentFrame, /*bIsResim*/false))
				{
					// If this scheduled command should get processed this step, inject it into the start of PushData:SimCommands to preserve call order and to cache the SimCommand in rewind history via PushData
					PushData.SimCommands.Insert(SimCallbackObject, 0);
					ScheduledSimCommands.RemoveAt(Idx, EAllowShrinking::No);
				}
			}
		}
		...
		for (FSimCallbackCommandObject*& SimCallbackObject : PushData.SimCommands)
		{
			...
			{
				// A scheduled command delayed for a later frame will leave a nullptr in the array
				continue;
			}

			if (SimCallbackObject->ShouldExecute(CurrentFrame, IsResimming()))
			{
				SimCallbackObject->SetSimAndDeltaTime_Internal(GetSolverTime(), MLastDt);
				SimCallbackObject->PreSimulate_Internal();
			}
			else if (SimCallbackObject->ShouldDelay(CurrentFrame, IsResimming()))
			{
				// Cache command to run on a future frame and null the pointer in this PushData
				MarshallingManager.EnqueueCommandScheduled_Internal(SimCallbackObject);
				SimCallbackObject = nullptr; // Stop tracking the command in this PushData
			}
		}
	...
	int32 FPBDRigidsSolver::DispatchTasksBufferPhysicsResults(int32 DispatchBatchIndex, int32 NumDirty, LambdaType ReturnHandle)
	{
		...
		// For performance reason collect on one threads the rigid proxies and on another one the geometry collection and the cluster union.
		// That way buffering results from rigid proxy could start after the first batch of rigid proxy collected, while geometry collection, cluster union 
		// and other batches of rigid cluster will be still collected.
		auto CollectRigidResults = [bIsResim, ReturnHandle](int32 StartIndex, int32 EndIndex, TArray<FSingleParticlePhysicsProxy*>& ActiveRigid)
		{
			for (int32 Idx = StartIndex; Idx < EndIndex; ++Idx)
			{
				THandleBase Handle = ReturnHandle(Idx);
				FTransientPBDRigidParticleHandle& DirtyParticle = static_cast<FTransientPBDRigidParticleHandle&>(Handle);
							...
							if (ClusterParticle->InternalCluster())
							{
								...
								if (bActivateChildren)
								{
									...
									for (IPhysicsProxyBase* ClusterProxy : Proxies)
									{
										...
										{
										case EPhysicsProxyType::SingleParticleProxy:
											ActiveRigid.Add((FSingleParticlePhysicsProxy*)ClusterProxy);
											break;
										case EPhysicsProxyType::GeometryCollectionType:
											break;
										default:
		...
		auto CollectGCAndClustersResults = [bIsResim, ReturnHandle](int32 StartIndex, int32 EndIndex,
		...
		return TaskDispatcher->CollectPhysicsResults(CollectRigidResults, CollectGCAndClustersResults, NumDirty, DispatchBatchIndex);
	}

	void FPBDRigidsSolver::BufferPhysicsResultsSingleThreaded(TArray<FSingleParticlePhysicsProxy*>& ActiveRigid, TArray<FGeometryCollectionPhysicsProxy*>& ActiveGC, TArray<FClusterUnionPhysicsProxy*>& ActiveClusterUnions)
	{
	...
	void FPBDRigidsSolver::BufferPhysicsResults()
	{
		...
		if (!(GSingleThreadedPhysics || CVars::bSingleThreadPushData))
		{
			using TSOA = typename TParticleView<FPBDRigidParticles>::TSOA;
			using THandle = typename TSOA::THandleType;
			using THandleBase = typename THandle::THandleBase;
			...
			TaskDispatcher->Initialize(DirtyParticles);
			...
			for (int32 ViewIndex = 0; ViewIndex < DirtyParticles.SOAViews.Num(); ++ViewIndex)
			{
				...
				{
					auto ReturnHandle = [CurHandlesArray](int32 Index) -> TParticleView<FPBDRigidParticles>::THandle::THandleBase
					{
					...
					check(NumHandles == NumParticles);
					DispatchBatchIndex += DispatchTasksBufferPhysicsResults<LambdaType>(DispatchBatchIndex, NumHandles, ReturnHandle);
				}
				else if (TSOA* Soa = SOAView.SOA)
				{
					auto ReturnHandle = [Soa](int32 Index) { return THandleBase(Soa, Index); };
					using LambdaType = decltype(ReturnHandle);
					DispatchBatchIndex += DispatchTasksBufferPhysicsResults<LambdaType>(DispatchBatchIndex, NumParticles, ReturnHandle);
				}
			}
			...
			check(TaskDispatcher->GetNumRigidBatches() == DispatchBatchIndex);
			TaskDispatcher->BufferRigidResults(PullData);
			TaskDispatcher->BufferGCResults(PullData, this);
			TaskDispatcher->BufferClusterResults(PullData);
		}
		else
		{
			// Those arrays are here only for the single threaded version
			TArray<FSingleParticlePhysicsProxy*> ActiveRigid;
			TArray<FGeometryCollectionPhysicsProxy*> ActiveGC;
			TArray<FClusterUnionPhysicsProxy*> ActiveClusterUnions;
			BufferPhysicsResultsSingleThreaded(ActiveRigid, ActiveGC, ActiveClusterUnions);
			{
				//we only fill this once per frame
				ensure(PullData->DirtyRigids.Num() == 0);
		...
		if (!(GSingleThreadedPhysics || CVars::bSingleThreadPushData))
		{
			TaskDispatcher->WaitTaskEndBufferResults();
		}
...
TRACE_COUNTER_SET(ChaosTraceCounter_##Name, Value)
	...
	void FPBDRigidsSolver::PostTickDebugDraw(FReal Dt) const
	{
		...
		// Don't debug draw resim frames
		if (GetEvolution()->IsResimming())
		{
			return;
		}
	...
	void FPBDRigidsSolver::SetParticleDynamicMisc(FPBDRigidParticleHandle* Rigid, const FParticleDynamicMisc& DynamicMisc)
	{
		...
		GetEvolution()->GetIslandManager().FlagParticleUpdated(Rigid->Handle());
	}

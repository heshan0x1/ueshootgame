...
void FGeometryCollectionPhysicsProxy::SetSleepingState(const Chaos::FPBDRigidsSolver& RigidsSolver)
{
	...
	{
		if (Handle && !Handle->Disabled() && Handle->IsDynamic())
		{
...
void FGeometryCollectionPhysicsProxy::InitializeBodiesPT(Chaos::FPBDRigidsSolver* RigidsSolver, typename Chaos::FPBDRigidsSolver::FParticlesType& Particles)
{
	...
	if (Parameters.Simulating && RestCollection)
	{
		...
		const TManagedArray<Chaos::FImplicitObjectPtr>* Implicits = DynamicCollection.FindAttribute<Chaos::FImplicitObjectPtr>(FGeometryDynamicCollection::ImplicitsAttribute, FTransformCollection::TransformGroup);
		if (!ensure(Implicits))
		{
			UE_LOG(LogChaos, Error, TEXT("Geometry collection %s - InitializeBodiesPT called with collision missing in the collection "), Owner ? *Owner->GetFullName(): TEXT("Unknown"));
		}
		...
		ParallelFor(NumEffectiveParticles, [&](int32 ParticleIndex)
		{
			...
			if (FClusterHandle* Handle = SolverParticleHandles[ParticleIndex])
			{
				...
				const bool bUseImplicit = (Implicits != nullptr) && (BuildGeometryForChildrenOnPT(Parameters) || (TransformGroupIndex == Parameters.InitialRootIndex));

				PopulateSimulatedParticle(
					Handle,
					Parameters.Shared,
					Simplicials? (*Simplicials)[TransformGroupIndex].Get(): nullptr,
					bUseImplicit? (*Implicits)[TransformGroupIndex] : nullptr,
					SimFilter,
					QueryFilter,
					ScaledMass,
					ScaledInertia,
		...
		for (int32 ParticleIndex = 0; ParticleIndex < SolverParticleHandles.Num(); ++ParticleIndex)
		{
			if (Chaos::FPBDRigidParticleHandle* Handle = SolverParticleHandles[ParticleIndex])
			{
				...
				Handle->SetPartialIslandSleepAllowed(Parameters.AllowPartialIslandSleep);
...
void FGeometryCollectionPhysicsProxy::CreateChildrenGeometry_Internal()
{
	...
	if (!bHasBuiltGeometryOnPT)
	{
		...
		if (Parameters.Simulating && ensure(RestCollection))
		{
			...
			const TManagedArray<FTransform>& MassToLocal = RestCollection->GetAttribute<FTransform>(MassToLocalAttributeName, FTransformCollection::TransformGroup);
			const TManagedArray<Chaos::FImplicitObjectPtr>* Implicits = PhysicsThreadCollection.FindAttribute<Chaos::FImplicitObjectPtr>(FGeometryDynamicCollection::ImplicitsAttribute, FTransformCollection::TransformGroup);
			...
			for (int32 ParticleIndex = 0; ParticleIndex < NumEffectiveParticles; ParticleIndex++)
			{
				...
				if (TransformIndex != Parameters.InitialRootIndex)
				{
					...
					if (ensure(Handle) && Handle->GetGeometry() == nullptr)
					{
						...
						SetImplicitToPTParticles(Handle, Parameters.Shared, Simplicial, Implicits? (*Implicits)[TransformIndex]: nullptr, SimFilter, QueryFilter, WorldTransform, CollisionParticlesPerObjectFraction);
...
Chaos::TPBDGeometryCollectionParticleHandle<Chaos::FReal, 3>* FGeometryCollectionPhysicsProxy::BuildNonClusters_Internal(const uint32 CollectionClusterIndex, Chaos::FPBDRigidsSolver* RigidsSolver, float Mass, Chaos::FVec3f Inertia, const Chaos::FUniqueIdx* ExistingIndex)
{
	...
	const bool bUseImplicit = (Implicits != nullptr) && (BuildGeometryForChildrenOnPT(Parameters) || (CollectionClusterIndex == Parameters.InitialRootIndex));
	...
	PopulateSimulatedParticle(
		...
		bUseImplicit? (*Implicits)[CollectionClusterIndex] : nullptr,
		SimFilter,
		QueryFilter,
		Mass,
		Inertia,
	...
	const Chaos::FUniqueIdx* ExistingIndex)
{
	...
	Chaos::TPBDRigidClusteredParticleHandle<Chaos::FReal, 3>* Parent =
		static_cast<Chaos::FPBDRigidsSolver*>(Solver)->GetEvolution()->GetRigidClustering().CreateClusterParticle(
			...
			Implicits? (*Implicits)[CollectionClusterIndex]: nullptr, // union from children if null
			&ParticleTM,
			ExistingIndex
			);
...
TBitArray<> FGeometryCollectionPhysicsProxy::CalculateClustersToCreateFromChildren(const FGeometryDynamicCollection& DynamicCollection, int32 NumTransforms)
{
	...
	if (bGeometryCollectionAlwaysGenerateGTCollisionForClusters)
	{
		...
		while (ChildrenToCheckForParentFix.Num())
		{
			...
			for (const int32 ParentToFixIndex : ParentToPotentiallyFix)
			{
				const bool bParentHasCollision = Implicits && (((*Implicits)[ParentToFixIndex] != nullptr) || ClustersToGenerate[ParentToFixIndex]);
				...
				if (!bParentHasCollision)
				{
					...
					{
						// defer if any of the children is a cluster with no collision yet generated 
						const bool bChildHasCollision = Implicits && (((*Implicits)[ChildIndex] != nullptr) || ClustersToGenerate[ChildIndex]);
...
int32 FGeometryCollectionPhysicsProxy::CalculateEffectiveParticles(const FGeometryDynamicCollection& DynamicCollection, int32 NumTransform, int32 InMaxSimulatedLevel, bool bEnableClustering, const UObject* Owner, TBitArray<>& EffectiveParticles)
{
	...
	for (int32 TransformIndex = 0; TransformIndex < NumTransform; ++TransformIndex)
	{
		...
		if (Level <= MaxSimulatedLevel || !bEnableClustering)
		{
			...
			const bool bHasGeometry = Implicits && (*Implicits)[TransformIndex].IsValid();
...
{
	extern bool bSingleThreadPushData;
}
...
static void UpdateParticleHandleTransform(Chaos::FPBDRigidsSolver& CurrentSolver, Chaos::FPBDRigidClusteredParticleHandle& Handle, const Chaos::FRigidTransform3& NewTransform)
{
	...
	if (Chaos::CVars::bSingleThreadPushData)
	{
...
bool FGeometryCollectionPhysicsProxy::PullNonInterpolatableDataFromSinglePhysicsState(const Chaos::FDirtyGeometryCollectionData& BufferData, bool bForcePullXRVW, const TBitArray<>* Seen)
{
	...
	for (int32 EntryIndex = 0; EntryIndex < CurrentResults.GetNumEntries(); EntryIndex++)
	{
		...
		// make sure we are not crashing if the transform index is invalid
		const bool bIsValidTransformIndex = FromTransformToParticleIndex.IsValidIndex(TransformGroupIndex);
		ensureMsgf(bIsValidTransformIndex, TEXT(" TransformGroupIndex is not valid : value = [%d] - Max valid TransformIndex = [%d]"), TransformGroupIndex, FromTransformToParticleIndex.Num());
		if (!bIsValidTransformIndex)
		{
			continue;
		}
...
void FGeometryCollectionPhysicsProxy::UpdateFilterData_External(const FCollisionFilterData& NewSimFilter, const FCollisionFilterData& NewQueryFilter)
{
	...
	Chaos::Filter::FCombinedShapeFilterData NewShapeFilter = Chaos::Filter::FShapeFilterBuilder::BuildFromLegacyShapeFilter(NewQueryFilter, NewSimFilter);
	UpdateFilterData_External(NewShapeFilter);
}
...
void FGeometryCollectionPhysicsProxy::UpdateFilterData_External(const Chaos::Filter::FCombinedShapeFilterData& NewShapeFilter)
{
	...
	// ShapeFilter members are read on both threads, these are const after initialization and are not updated here.
	for (int32 Index = 0; Index < NumEffectiveParticles; ++Index)
	{
		FParticle* P = GTParticles[Index].Get();
		if (P != nullptr)
		{
			...
			{
				Chaos::FPerShapeData* Shape = Shapes[ShapeIndex].Get();
				Shape->SetCombinedShapeFilterData(NewShapeFilter);
			}
		}
	}

	ExecuteOnPhysicsThread(*this, [this, NewShapeFilter]()
		{
			SetFilterData_Internal(NewShapeFilter);
		});
}
...
void FGeometryCollectionPhysicsProxy::SetFilterData_Internal(const Chaos::Filter::FCombinedShapeFilterData& NewShapeFilter)
{
	...
	Chaos::Filter::FShapeFilterBuilder::GetLegacyShapeFilter(NewShapeFilter, Parameters.QueryFilterData, Parameters.SimulationFilterData);
		...
		for (const FParticleCollisionFilterData& Data : PerParticleData)
		{
			if (Data.bIsValid && Data.ParticleIndex != INDEX_NONE)
			{
				...
				if (ParticleIndex != INDEX_NONE)
				{
					...
					Interface.UpdateCombinedShapeFilterData(ParticleView, Chaos::Filter::FShapeFilterBuilder::BuildFromLegacyShapeFilter(Data.QueryFilter, Data.SimFilter));

...
// =====================================================================================================================
// NetObjectGridFilter.cpp —— UNetObjectGridFilter / UNetObjectGridWorldLocFilter 的实现。
// 算法详细说明见同名 .h 头部。本 .cpp 关注：
//   - Cell 维护（Add/Remove/UpdateCellInfoForObject 三组互补操作，特别是 disjoint vs overlap 的对称增删）；
//   - Filter() 主路径中两条分支：精确距离 vs Cell 粗判；
//   - PerConnectionInfo 中 RecentCells（视点 cell 的滚动窗口）与 RecentObjectFrameCount（per-object 滞后计数）。
// =====================================================================================================================
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(NetObjectGridFilter)

// 初始化：声明空间过滤 trait + 强引用 Config + 分配位图、读世界边界与 cull 上限。
void UNetObjectGridFilter::OnInit(const FNetObjectFilterInitParams& Params)
{
	...
	// PerConnectionInfos 索引为 ConnectionId（连接 ID 从 1 起，故多分配 1 槽）。
...
// 索引上限扩容：仅需扩展 AssignedObjectInfoIndices 位图（ObjectInfos 是 ChunkedArray，按需追加）。
...
// 新连接接入：清空对应槽位（默认构造 FPerConnectionInfo 空容器）。
...
// Profile 查找：找不到匹配 Profile 时返回 Config 默认值；找到则返回该 Profile 的滞后帧数。
...
// 添加对象到 Grid 索引：
//   1) 派生类 BuildObjectInfo 决定位置数据来源（RepTag 状态 / WorldLocations）；
//   2) 分配 FPerObjectInfo 槽位；
//   3) AddCellInfoForObject 计算 CellBox 并把对象写入所有覆盖到的 Cell；
//   4) cull 距离过大（超过 MaxNetCullDistance）则回滚——会被外层视为"始终相关"。
...
// 移除对象：
//   1) 把对象从所有连接的 RecentObjectFrameCount Map 中删除（避免悬挂索引）；
//   2) RemoveCellInfoForObject 把对象从所有覆盖到的 Cell 中移除；
//   3) 派生类 OnObjectRemoved 钩子清理外挂数据；
//   4) 释放 FPerObjectInfo 槽位。
...
// PreFilter：每帧自增 FrameIndex（Cell 时间戳基准）+ 重置 CSV 统计。
...
// 主过滤：详见文件头算法说明。
//   - 维护连接的 RecentCells（视点 cell 的滚动窗口，含时间戳）；
//   - 候选集来自这些 Cell 中的对象；
//   - 精确距离路径：DistSq 比较通过则刷新 RecentObjectFrameCount，并按帧数倒计；
//   - 近似路径：候选集对象一律 Allow（无滞后机制，依赖 ViewPosRelevancyFrameCount 在 Cell 维度做滞后）。
void UNetObjectGridFilter::Filter(FNetObjectFilteringParams& Params)
{
	...
	// 步骤 1：把当前帧每个分屏视点所在 Cell 加入 NewCells；同名 Cell 从 PrevCells 删除（仅留下需要"过期"判断的旧 Cell）。
	...
	if (!Config->bUseExactCullDistance)
	{
		...
		// 步骤 2（仅近似路径）：把仍在过期窗口内的旧 Cell 也并入 NewCells，平滑跨 Cell 抖动。
	 ...
	 */
	// 备注：对象会被多次置位（横跨多 Cell），但写位图比维护 Set 更便宜，权衡后选用位图。

	if (Config->bUseExactCullDistance)
	{
		// 精确路径：遍历相关 Cell 中的对象，对每个对象做距离平方比较；通过则刷新滞后计数。
#if UE_NET_IRIS_CSV_STATS
		...
		// 滞后倒计：> 0 则保留并 -1，否则从 Map 移除（边迭代边删，TIterator::RemoveCurrent 安全）。
...
// PostFilter：把 cull 测试的耗时与样本数推送到 CSV。
...
// 找下一个空闲槽位；若位图已满则按 chunk 扩容 ObjectInfos。位图存储索引边界，TChunkedArray 真正持有 PerObjectInfo。
...
// 释放槽位：清位 + 重置 PerObjectInfo（FPerObjectInfo 默认构造把所有字段清零）。
...
// 首次添加对象：拉取 cull 距离与位置 → 算 CellBox → 把对象注册到所有覆盖 Cell。
// 若 cull 超过 MaxNetCullDistance，返回 false（不要因为大对象不断跨 Cell 影响整体性能）。
bool UNetObjectGridFilter::AddCellInfoForObject(const FObjectLocationInfo& ObjectLocationInfo, const UE::Net::FReplicationInstanceProtocol* InstanceProtocol)
{
	...
	// Set cull distance
	{
		// 默认值兜底，再让派生类 UpdateObjectInfo 覆盖（可能从 WorldLocations 读到精确值）。
	...
	// 双层循环把对象索引写入 NewCellBox 范围内每个 Cell 的 ObjectIndices Set。
	{
		FCellCoord Coord;
...
// 移除：与 AddCellInfoForObject 镜像，遍历 PerObjectInfo.CellBox 范围逐个 Cell 删除对象索引。
...
// 更新（对象移动）：算新 CellBox，与旧 CellBox 比较；
//   - 完全不相交（Disjoint）：旧框全删，新框全加；
//   - 相交：算 Overlap，旧框中"非 Overlap"删，新框中"非 Overlap"加（最少改动）。
void UNetObjectGridFilter::UpdateCellInfoForObject(const FObjectLocationInfo& ObjectLocationInfo, const UE::Net::FReplicationInstanceProtocol* InstanceProtocol)
{
	...
	if (NewCellBox != PrevCellBox)
	{
			...
			// Add object to new cells.
			{
				...
				for (int32 X = NewCellBox.MinX, EndX = NewCellBox.MaxX + 1; X < EndX; ++X)
				{
					...
					for (int32 Y = NewCellBox.MinY, EndY = NewCellBox.MaxY + 1; Y < EndY; ++Y)
					{
						...
						// 调试快照：UpdateCell 是热点，崩溃时可由此还原参数。
		...
		else
		{
			 ...
			 */
			// 朴素增量更新：先算 Overlap，再分别遍历 Prev 与 New 的全集，跳过 Overlap 部分。
			FCellBox Overlap;
			...
			// Remove object from previous cells.
			{
				...
				for (int32 X = PrevCellBox.MinX, EndX = PrevCellBox.MaxX + 1; X < EndX; ++X)
				{
					...
					for (int32 Y = PrevCellBox.MinY, EndY = PrevCellBox.MaxY + 1; Y < EndY; ++Y)
					{
						...
						FCellObjects* Cell = Cells.Find(Coord);
						checkSlow(Cell != nullptr);
						// TSet::Remove(Value) —— 按值删除（TSet 内键即值）。
						Cell->ObjectIndices.Remove(ObjectIndex);
					}
				}
			}
...
// 转交派生类：在 ObjectInfo 上把"位置"与"cull 距离"刷新到最新（GridWorldLocFilter 从 WorldLocations 读取）。
...
// 由 Position±CullDistance 推算覆盖的 CellBox。
//   - 用 MaxNetCullDistance 约束 cull 距离上界（兼容性 max-min 写法保证 cull 不超过对象自身值）；
//   - 用 MinWorldPos / MaxWorldPos 约束 BBox 不越界（避免 int32 溢出）；
//   - 大 LWC 世界（最大 ~8.8e12）下 cell 至少 ~4500 才能不溢出 int32。
...
// 单点 → 单 Cell（用于 viewer 当前所在 Cell 的查询）。
...
// 调试输出：打印 2D 距离、cull 距离、剩余滞后帧数、Z 距离与位置。
...
// 缓存 WorldLocations 引用：之后的 PreFilter / UpdateObjectInfo / BuildObjectInfo 都依赖它。
...
// PreFilter：批量处理本帧位置变化的对象（脏位置位图与本 Filter 接管的对象集合取交集），逐个 UpdateCellInfoForObject。
// 不依赖 dirty state poll —— 由 Bridge 已把世界位置写入 WorldLocations，这里直接消费。
...
// 从 WorldLocations 读取最新位置 + cull 距离写入 PerObjectInfo（仅当对象走 WorldLocations 路径）。
void UNetObjectGridWorldLocFilter::UpdateObjectInfo(UNetObjectGridFilter::FPerObjectInfo& PerObjectInfo, const UNetObjectGridFilter::FObjectLocationInfo& ObjectLocationInfo, const UE::Net::FReplicationInstanceProtocol* InstanceProtocol)
{
	...
	const TOptional<UE::Net::FWorldLocations::FWorldInfo> Info = WorldLocations->GetWorldInfo(PerObjectInfo.ObjectIndex);
...
// 添加对象时，标记走 WorldLocations 路径（offset/index 都置为 Invalid 哨兵）。

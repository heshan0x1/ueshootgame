...
#ifndef UE_NET_OBJECTREFERENCECACHE_ASYNCLOAD_DEBUG
...
static bool bForceAsyncLoading = false;
FAutoConsoleVariableRef CVarIrisForceAsyncLoading(TEXT("net.Iris.AsyncLoading.ForceEnable"), bForceAsyncLoading, TEXT("Force the async loading path even in Editor/PIE."), ECVF_Default);
#endif

#if UE_NET_OBJECTREFERENCECACHE_ASYNCLOAD_DEBUG
static FString IrisAsyncLoadingFailPackageName;
...
static bool bIrisAsyncLoadingFailNextLoad;
...
static bool bIrisAsyncLoadingFailAllLoads;
...
void FObjectReferenceCache::BindRemoteReference(FNetRefHandle RefHandle, const UObject* Object)
{
	...
	UE_LOG_REFERENCECACHE(Verbose, TEXT("%hs: Binding Handle: %s to Object: %s (0x%p)" ), __FUNCTION__, *RefHandle.ToString(), *GetNameSafe(Object), Object);
	...
	if (FNetRefHandle* ExistingHandle = ObjectToNetReferenceHandle.Find(Object))
	{
		if (FCachedNetObjectReference* ExistingCachedObject = ReferenceHandleToCachedReference.Find(*ExistingHandle))
		{
			...
			{
				UE_LOG_REFERENCECACHE(Warning, TEXT("%hs: Detected conflicting Ref %s: Received: %s, Object: %s (0x%p)"), __FUNCTION__, *ExistingCachedObject.NetRefHandle.ToString(),
	...
	{
		UE_LOG_REFERENCECACHE(Verbose, TEXT("%hs: Updating handle:%s to Object: %s (0x%p). Previous ptr: (0x%p)"), __FUNCTION__, *RefHandle.ToString(), *GetNameSafe(Object), Object, CacheObjectPtr->ObjectKey);
	...
	else
	{
		...
		UE_LOG_REFERENCECACHE(Verbose, TEXT("%hs: Adding new cache entry for: %s with Object: %s (0x%p)."), __FUNCTION__, *RefHandle.ToString(), *GetNameSafe(Object), Object);
...
void FObjectReferenceCache::AddPreRegisteredReference(FNetRefHandle RefHandle, const UObject* Object)
{
	if (!IsAuthority())
	{
		...
		BindRemoteReference(CompleteRefHandle, Object);
...
void FObjectReferenceCache::RemoveReference(FNetRefHandle RefHandle, const UObject* Object)
{
	...
	const bool bIsDynamic = RefHandle.IsDynamic();
	if (!bIsDynamic)
	{
		ensureMsgf(!bIsDynamic, TEXT("%hs not allowed on static handles: %s - %s"), __FUNCTION__, *RefHandle.ToString(), *GetNameSafe(Object));
		return;
	}
...
bool FObjectReferenceCache::IsNetRefHandlePending(FNetRefHandle NetRefHandle, const FPendingBatchHolder& PendingBatchHolder) const
{
	// Check Outer chain
	while (NetRefHandle.IsValid())
	{
		// Need lambda to figure this one out
		if (PendingBatchHolder.Contains(NetRefHandle))
		{
			return true;
		}
...
UObject* FObjectReferenceCache::ResolveObjectReferenceHandleInternal(FNetRefHandle RefHandle, const FNetObjectResolveContext& ResolveContext, bool& bOutMustBeMapped)
{
	...
	if (Object != nullptr)
	{
		...
		UE_LOG_REFERENCECACHE(VeryVerbose, TEXT("ObjectReferenceCache::ResolveObjectRefererenceHandle Found existing object for %s for Object: %s Pointer: 0x%p"), *RefHandle.ToString(), ToCStr(Object->GetPathName()), Object);
		...
		if (bIsPackage)
		{
			...
			if (ShouldAsyncLoad() && !ResolveContext.bForceSyncLoad)
			{
				if (!PendingAsyncLoadRequests.Contains(ObjectPathName))
				{
					...
					StartAsyncLoadingPackage(*CacheObjectPtr, ObjectPathName, ResolveContext.AsyncLoadingPriority, RefHandle, false);
	...
	if (bIsPackage)
	{
			...
			&& !Package->HasAnyPackageFlags(TreatAsLoadedFlags)) //TODO: dependencies of CompiledIn could still be loaded asynchronously. Are they necessary at this point??
		{
			if (ShouldAsyncLoad() && !ResolveContext.bForceSyncLoad && Package->HasAnyInternalFlags(EInternalObjectFlags_AsyncLoading))
			{
				...
				StartAsyncLoadingPackage(*CacheObjectPtr, ObjectPathName, ResolveContext.AsyncLoadingPriority, RefHandle, true);
	...
	if (Reference.PathToken.IsValid())
	{
		...
		if (ResolvedToken)
		{
			...
			if (Reference.GetRefHandle().IsValid())
			{
				...
				ResolvedObject = ReplicatedOuter ? StaticFindObject(UObject::StaticClass(), ReplicatedOuter, ToCStr(ObjectPath), EFindObjectFlags::None) : nullptr;
			...
			{
				ResolvedObject = StaticFindObject(UObject::StaticClass(), nullptr, *ObjectPath, EFindObjectFlags::None);
...
void FObjectReferenceCache::ReadFullReferenceInternal(FNetSerializationContext& Context, FNetObjectReference& OutObjectRef, uint32 RecursionCount)
{
	...
	{
		UE_LOG_REFERENCECACHE(Error, TEXT("ReadFullReferenceInternal: Bitstream overflow"));
...
void FObjectReferenceCache::ReadFullReference(FNetSerializationContext& Context, FNetObjectReference& OutRef)
{
	...
	if (const bool bIsClientAssignedReference = Reader->ReadBool())
	{
		...
		// We expect a valid RelativePath
		if (!RelativePath.IsValid())
		{
			UE_LOG_REFERENCECACHE(Error, TEXT("ReadFullReference: Invalid relative path"));
...
void FObjectReferenceCache::ReadReference(FNetSerializationContext& Context, FNetObjectReference& OutRef)
{
	...
	if (const bool bIsClientAssignedReference = Reader->ReadBool())
	{
		...
		if (!RelativePath.IsValid())
		{
			UE_LOG_REFERENCECACHE(Error, TEXT("ReadReference: Invalid relative path"));
			Context.SetError(GNetError_InvalidValue);
		}
	...
	if (Reader->IsOverflown())
	{
		UE_LOG_REFERENCECACHE(Error, TEXT("ReadReference: Bitstream overflow"));
		Context.SetError(GNetError_BitStreamOverflow);
		OutRef = FNetObjectReference();
		return;
	}
...
void FObjectReferenceCache::AddPendingExports(FNetSerializationContext& Context, TArrayView<const FObjectNetSerializerQuantizedReferenceStorage> ExportsView) const
{
	if (FNetExportContext* ExportContext = Context.GetExportContext())
	{
		for (const FObjectNetSerializerQuantizedReferenceStorage& Reference : ExportsView)
		{
			const FQuantizedObjectReference& InternalReference = *reinterpret_cast<const FQuantizedObjectReference*>(&Reference.Storage);

			if (InternalReference.IsNetReference())
			{
				AddPendingExport(*ExportContext, InternalReference.NetReference);
			}
		}
	}
}
	...
	// Force exports to be written
	{
		...
		for (const FNetObjectReference& Reference : NetObjectReferenceExportsView)
		{
			...
			if ((bIsClientAssigned && !ExportContext->IsExported(Reference.GetPathToken())) || (IsAuthority() && Reference.CanBeExported() && !ExportContext->IsExported(Handle)))
			{
				...
				//UE_LOG(LogIrisReferences, Verbose, TEXT("WritePendingExports for %s : Export[%d] %s"), *NetRefHandleManager->PrintObjectFromIndex(ObjectIndex), CountExports, *NetRefHandleManager->PrintObjectFromNetRefHandle(Handle));
...
bool FObjectReferenceCache::ReadExports(const FNetRefHandle& NetObjectHandle, FNetSerializationContext& Context, TArray<FNetRefHandle>* MustBeMappedExports, EIrisAsyncLoadingPriority& OutIrisAsyncLoadingPriority)
{
	...
	{
		Context.SetError(GNetError_InvalidValue);
		UE_LOG_REFERENCECACHE(Error, TEXT("ReadExports: Null MustBeMappedExports"));
		ensure(false);
		return false;
	}
	...
	if (!Context.HasErrorOrOverflow())
	{
		ReadMustBeMappedExports(NetObjectHandle, Context, MustBeMappedExports, OutIrisAsyncLoadingPriority);
	}
...
bool FObjectReferenceCache::WriteMustBeMappedExports(FNetSerializationContext& Context, FInternalNetRefIndex ObjectIndex, TArrayView<const FNetObjectReference> ExportsView) const
{
	...
	if (IsAuthority() && ShouldAsyncLoad() && ExportContext)
	{
		...
		const EIrisAsyncLoadingPriority IrisAsyncLoadingPriority = NetRefHandleManager->GetIrisAsyncLoadingPriority(ObjectIndex);

		// Only write the loading priority before the first reference
		bool bHasWrittenPriority = false;
		...
		for (const FNetObjectReference& Reference : ExportsView)
		{
			...
			if (CachedObject && !CachedObject->bNoLoad)
			{
				...
				if (!bHasWrittenPriority)
				{
					bHasWrittenPriority = true;
					Writer.WriteBits((uint32)IrisAsyncLoadingPriority, GetIrisAsyncLoadingPriorityBits());
				}

				WriteNetRefHandle(Context, Handle);

#if UE_NET_IRIS_CSV_STATS
				UE_LOG(LogIrisReferences, VeryVerbose, TEXT("WriteMustBeMappedExports for %s. MustBeMapped[%d] %s (%s)"), *NetRefHandleManager->PrintObjectFromIndex(ObjectIndex), CountExports, *GetNameSafe(CachedObject->Object.Get()), *Handle.ToString());
#endif
...
#if UE_NET_IRIS_CSV_STATS
		UE_CLOG(CountExports>0, LogIrisReferences, VeryVerbose, TEXT("WriteMustBeMappedExports for %s. Wrote %d exports with priority: %s"), *NetRefHandleManager->PrintObjectFromIndex(ObjectIndex), CountExports, LexToString(IrisAsyncLoadingPriority));
#endif
...
void FObjectReferenceCache::ReadMustBeMappedExports(const FNetRefHandle& NetObjectHandle, FNetSerializationContext& Context, TArray<FNetRefHandle>* MustBeMappedExports, EIrisAsyncLoadingPriority& OutIrisAsyncLoadingPriority)
{
	if (!MustBeMappedExports)
	{
		Context.SetError(GNetError_InvalidValue);
		UE_LOG_REFERENCECACHE(Error, TEXT("ReadMustBeMappedExports: Null MustBeMappedExports"));
		ensure(false);
		return;
	}
	...
	if (bHasExportsToRead)
	{	
		OutIrisAsyncLoadingPriority = static_cast<EIrisAsyncLoadingPriority>(Reader.ReadBits(GetIrisAsyncLoadingPriorityBits()));
	}
	...
	while (bHasExportsToRead && !Context.HasErrorOrOverflow())
	{
		...
		UE_LOG(LogIrisReferences, VeryVerbose, TEXT("ReadMustBeMappedExports for %s. MustBeMapped[%d] %s"), *NetObjectHandle.ToString(), MustBeMappedExports->Num(), *MustBeMappedHandle.ToString());

		MustBeMappedExports->Add(MustBeMappedHandle);
	}

	UE_CLOG(MustBeMappedExports->Num() > 0, LogIrisReferences, VeryVerbose, TEXT("ReadMustBeMappedExports for %s. Read %d references with priority: %s"), *NetObjectHandle.ToString(), MustBeMappedExports->Num(), LexToString(OutIrisAsyncLoadingPriority));
}
...
void FObjectReferenceCache::StartAsyncLoadingPackage(FCachedNetObjectReference& CacheObject, FName PackagePath, TAsyncLoadPriority AsyncLoadingPriority, const FNetRefHandle RefHandle, const bool bWasAlreadyAsyncLoading)
{
	...
	if (!bIsPrioritySet)
	{
		ensureMsgf(false, TEXT("AsyncLoadingPriorty wasn't set in the ResolveContext. Using default value as fallback"));
		AsyncLoadingPriority = ConvertAsyncLoadingPriority(EIrisAsyncLoadingPriority::Default);
	}

	UE_LOG(LogIrisReferences, Verbose, TEXT("StartAsyncLoadingPackage for %s: Package: %s (Async loading priority: %d)"), *NetRefHandleManager->PrintObjectFromNetRefHandle(RefHandle), *PackagePath.ToString(), AsyncLoadingPriority);
...
#if UE_NET_OBJECTREFERENCECACHE_ASYNCLOAD_DEBUG
	if (bIrisAsyncLoadingFailAllLoads || bIrisAsyncLoadingFailNextLoad || (IrisAsyncLoadingFailPackageName.Len() > 0 && PackagePath.ToString().Find(IrisAsyncLoadingFailPackageName) != INDEX_NONE))
	{
		bIrisAsyncLoadingFailNextLoad = false;
		// Not thread safe. We don't want to load the package so we call our fail callback immediately.
		AsyncPackageForcedFailCallback(PackagePath, nullptr, EAsyncLoadingResult::Failed);
	}
	else
#endif
	{
...
void FObjectReferenceCache::AsyncPackageForcedFailCallback(const FName& PackageName, UPackage* Package, EAsyncLoadingResult::Type Result)
{
#if UE_NET_OBJECTREFERENCECACHE_ASYNCLOAD_DEBUG
	UE_LOG_REFERENCECACHE(Warning, TEXT("AsyncPackageCallback: Force fail async package load. Path: %s"), ToCStr(PackageName.ToString()));

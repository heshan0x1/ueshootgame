...
{	
	...
	// We also reserve room for the MemberPollMask
	const SIZE_T MemberPollMaskMemberSize = FNetBitArrayView::CalculateRequiredWordCount(Members.Num()) * sizeof(FNetBitArrayView::StorageWordType);

	// Update context
	Context.External.Alignment = FMath::Max(Context.External.Alignment, ChangeMaskMemberAlignment);
	Context.External.Size = Descriptor->ChangeMasksExternalOffset + ChangeMaskMemberSize + ConditionalChangeMaskMemberSize + MemberPollMaskMemberSize;
}
...
void FPropertyReplicationStateDescriptorBuilder::BuildMemberTraitsDescriptors(FReplicationStateMemberTraitsDescriptor* MemberTraitsDescriptors, FReplicationStateDescriptor* Descriptor, const FBuilderContext& Context) const
{
	...
	for (const FMemberProperty& Info : Members)
	{
		...
		CurrentMemberTraitsDescriptor->Traits |= (EnumHasAnyFlags(Info.Traits, EMemberPropertyTraits::HasPushBasedDirtiness) ? EReplicationStateMemberTraits::HasPushBasedDirtiness : EReplicationStateMemberTraits::None);
...
EReplicationStateTraits FPropertyReplicationStateDescriptorBuilder::BuildReplicationStateTraits(const FBuilderContext& Context) const
{
	...
	if (EnumHasAnyFlags(PropertyTraits, EMemberPropertyTraits::HasPushBasedDirtiness))
	{
		Traits |= EReplicationStateTraits::HasPushBasedDirtiness;

		// Only set the HasFullPushBasedDirtiness trait if all properties are push based. An alternative could be to split into multiple states.
		if (EnumHasAnyFlags(SharedPropertyTraits, EMemberPropertyTraits::HasPushBasedDirtiness))
		{
			Traits |= EReplicationStateTraits::HasFullPushBasedDirtiness;
		}
	}
...
{ 
	bool bResult = Condition == COND_InitialOnly;
...
SIZE_T FReplicationStateDescriptorBuilder::CreateDescriptorsForClass(FResult& CreatedDescriptors, UClass* InObjectClass, const FParameters& Parameters)
{
	...
	// Use the template as the default key or the class CDO if there is no template.
	const UObject* DefaultStateSource = Parameters.DefaultStateSource ? Parameters.DefaultStateSource : InObjectClass->GetDefaultObject();

	// Check registry first to see if we already have created descriptors for this class
	if (Parameters.DescriptorRegistry)
	{
		if (const FReplicationStateDescriptorRegistry::FDescriptors* Result = Parameters.DescriptorRegistry->Find(DefaultStateSource, InObjectClass))
		{
			UE_LOG_DESCRIPTORBUILDER(VeryVerbose, TEXT("Found existing descriptors for Class %s : DescriptorKey: %s ptr: 0x%p"), *InObjectClass->GetName(), *DefaultStateSource->GetPathName(), DefaultStateSource);
	...
	// Add special Actor properties
	{
		...
		UClass* ActorClass = CastChecked<UClass>(StaticFindObject(UClass::StaticClass(), nullptr, TEXT("/Script/Engine.Actor"), EFindObjectFlags::ExactClass));
	...
	const uint8* DefaultStateSourceData = reinterpret_cast<const uint8*>(DefaultStateSource);
	...
	{
		UE_LOG_DESCRIPTORBUILDER(VeryVerbose, TEXT("Registering descriptors for Class %s : DescriptorKey: %s ptr: 0x%p"), *InObjectClass->GetName(), *DefaultStateSource->GetPathName(), DefaultStateSource);
		Parameters.DescriptorRegistry->Register(DefaultStateSource, InObjectClass, CreatedDescriptors);
	}

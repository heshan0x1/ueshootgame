...
public:
	...
	inline bool IsActiveTilesGenerationEnabled() const{ return bGenerateNavigationOnlyAroundNavigationInvokers; }
...
public:
	...
	inline const TArray<FNavDataConfig>& GetSupportedAgents() const { return SupportedAgents; }
	...
	inline bool SupportsNavigationGeneration() const { return bSupportRebuilding; }
...
public:
	...
	inline bool HasPendingUpdateForElement(FNavigationElementHandle Element) const
	{
...
public:
	...
	// @todo document
	inline bool IsNavigationBuildingLocked(uint8 Flags = ~0) const { return (NavBuildingLockFlags & Flags) != 0; }

	/** check if building is permanently locked to avoid showing navmesh building notify (due to queued dirty areas) */
	inline bool IsNavigationBuildingPermanentlyLocked() const
	{
		return (NavBuildingLockFlags & ~ENavigationBuildLock::InitialLock) != 0; 
	}

	/** check if navigation octree updates are currently ignored */
	inline bool IsNavigationOctreeLocked() const { return DefaultOctreeController.IsNavigationOctreeLocked(); }
...
public:
	...
	inline virtual bool IsWorldInitDone() const override { return bWorldInitDone; }
	inline bool IsInitialized() const { return bWorldInitDone; }

	inline FNavigationSystemRunMode GetRunMode() const { return OperationMode; }
	...
	*	value, but can be overridden per nav sys instance */
	...
	inline bool IsNavigationRegisterLocked() const { return NavUpdateLockFlags != 0; }
	inline bool IsNavigationUnregisterLocked() const { return NavUpdateLockFlags && !(NavUpdateLockFlags & ENavigationLockReason::AllowUnregister); }
	inline bool IsNavigationUpdateLocked() const { return IsNavigationRegisterLocked(); }
	inline void AddNavigationUpdateLock(uint8 Flags) { NavUpdateLockFlags |= Flags; }
	inline void RemoveNavigationUpdateLock(uint8 Flags) { NavUpdateLockFlags &= ~Flags; }

	NAVIGATIONSYSTEM_API void UpdateLevelCollision(ULevel* InLevel);

#endif // WITH_EDITOR

	inline bool IsSetUpForLazyGeometryExporting() const { return bGenerateNavigationOnlyAroundNavigationInvokers; }
	...
	static inline bool ShouldUpdateNavOctreeOnComponentChange()
	{
	...
	static inline bool IsNavigationSystemStatic()
	{
#if WITH_EDITOR

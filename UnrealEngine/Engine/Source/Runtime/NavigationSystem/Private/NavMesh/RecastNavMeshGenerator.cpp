...
void FRecastTileGenerator::GatherNavigationDataGeometry(const TSharedRef<FNavigationRelevantData, ESPMode::ThreadSafe>& ElementDataRef, UNavigationSystemV1& NavSys, const FNavDataConfig& OwnerNavDataConfig, const bool bGeometryChanged)
{
	...
	// Not using the ternary operator to avoid a lvalue-to-rvalue conversion which causes copies of FCompositeNavModifier even when no meta area is used.
	// This is to avoid performance hit when using FCompositeNavModifier with a lot of internal content.
	TOptional<FCompositeNavModifier> Storage;
	const FCompositeNavModifier* ModifierInstancePtr;
	if (ElementData.Modifiers.HasMetaAreas())
	{
		Storage = ElementData.Modifiers.GetInstantiatedMetaModifier(&OwnerNavDataConfig, ElementData.SourceElement->GetWeakUObject());
		ModifierInstancePtr = &Storage.GetValue();
	}
	else
	{
		ModifierInstancePtr = &ElementData.Modifiers;
	}
	const FCompositeNavModifier& ModifierInstance = *ModifierInstancePtr;
...
void FRecastTileGenerator::AppendModifier(const FCompositeNavModifier& Modifier, const FNavDataPerInstanceTransformDelegate& InTransformsDelegate)
{
	...
	if (IsTileDebugActive())
	{
		...
		for (const FTransform& Transform : ModifierElement.PerInstanceTransform)
		{
			StrBuilder += FString::Printf(TEXT(" %s"), *Transform.ToString());
		}
		UE_LOG(LogNavigation, Log, TEXT("    Modifier transforms [%d]:%s"), ModifierElement.PerInstanceTransform.Num(), StrBuilder.ToString());

		for (const FAreaNavModifier& AreaNavModifier : ModifierElement.Areas)
		{
			UE_LOG(LogNavigation, Log, TEXT("    Adding area modifier:\n"
										"                           Type: %s\n"
										"                           ApplyMode: %s\n"
										"                           AreaClass: %s\n"
										"                           Bounds: %s"),
			*LexToString(AreaNavModifier.GetShapeType()),
			*LexToString(AreaNavModifier.GetApplyMode()),
			*AreaNavModifier.GetAreaClass()->GetName(),
			*AreaNavModifier.GetBounds().ToString());
		}
	}
#endif
...
{
	auto AddLinkSharedLambda = [&OutGeneratedLinks](const dtNavLinkBuilderJumpConfig& config) -> FGeneratedNavigationLink&
	{
	...
	auto AddBidirectionalLinkLambda = [&AddLinkSharedLambda](const dtNavLinkBuilderJumpConfig& config, const dtReal* posA, const dtReal* posB)
	{
	...
	auto AddDirectionalLinksLambda = [&AddLinkSharedLambda](const dtNavLinkBuilderJumpConfig& config, const dtReal* posA, const dtReal* posB)
	{
	...
	for (const dtNavLinkBuilder::JumpLink& link : linkBuilder.m_links)
	{
		...
		const dtNavLinkBuilderJumpConfig& jumpConfig = linkBuilderConfig.jumpConfigs[link.configIndex];
		if (!ensure(jumpConfig.enabled))
			continue;

		const bool bBidirectionalJumpDownLinks = jumpConfig.downDirArea == jumpConfig.upDirArea;
		TFunctionRef<void(const dtNavLinkBuilderJumpConfig&, const dtReal*, const dtReal*)> AddLinkLambda =
			bBidirectionalJumpDownLinks ?
				TFunctionRef<void(const dtNavLinkBuilderJumpConfig&, const dtReal*, const dtReal*)>(AddBidirectionalLinkLambda) :
				TFunctionRef<void(const dtNavLinkBuilderJumpConfig&, const dtReal*, const dtReal*)>(AddDirectionalLinksLambda);
		
		if (jumpConfig.linkBuilderFlags & DT_NAVLINK_CREATE_CENTER_POINT_LINK)
		{
			...
			AddLinkLambda(jumpConfig, midA, midB);
		}

		if (jumpConfig.linkBuilderFlags & DT_NAVLINK_CREATE_EXTREMITY_LINKS)
		{
			dtReal posA[3];
			dtReal posB[3];
			...
			AddLinkLambda(jumpConfig, posA, posB);
                                                   ...
                                                   const dtTileCacheContourSet& lcset, TArray<FGeneratedNavigationLink>& OutGeneratedLinks) const
{
	...
	if (IsTileDebugActive() && (TileDebugSettings.LinkGenerationDebugFlags != 0))
	{
		...
		DebugConfig = TileDebugSettings.LinkGenerationSelectedConfig;
	...
	linkBuilderConfig.jumpConfigs = TileConfig.JumpConfigs;
	for (dtNavLinkBuilderJumpConfig& jumpConfig : linkBuilderConfig.jumpConfigs)
	{
	...
	const bool bAllEdges = DebugEdge == -1;
	const bool bAllConfigs = DebugConfig == -1;
	
	if (bAllEdges && bAllConfigs)
	{
		for (const dtNavLinkBuilderJumpConfig& jumpConfig : linkBuilderConfig.jumpConfigs)
		{
			if (jumpConfig.enabled)
			{
				TRACE_CPUPROFILER_EVENT_SCOPE(RecastBuildLinks_JumpConfig);
				linkBuilder.buildForAllEdges(BuildContext, linkBuilderConfig, jumpConfig);
			}
		}

#if RECAST_INTERNAL_DEBUG_DATA
		duDebugDrawNavLinkBuilder(dd, linkBuilder, DebugFlags, nullptr /*edgeSampler*/);
	}
	else
	{
		for (const dtNavLinkBuilderJumpConfig& jumpConfig : linkBuilderConfig.jumpConfigs)
		{
			if (jumpConfig.enabled && (bAllConfigs || jumpConfig.ConfigIndex == DebugConfig))
			{
				if (bAllEdges)
				{
					linkBuilder.buildForAllEdges(BuildContext, linkBuilderConfig, jumpConfig);
				...
				{
					dtNavLinkBuilder::EdgeSampler sampler;
					linkBuilder.debugBuildEdge(linkBuilderConfig, DebugEdge, sampler, jumpConfig);
					duDebugDrawNavLinkBuilder(dd, linkBuilder, DebugFlags, &sampler);
				}
			}
		}
#endif // RECAST_INTERNAL_DEBUG_DATA
	...
	AddGeneratedLinks(OutGeneratedLinks, linkBuilderConfig, linkBuilder);
...
int32 CalculateMaxTilesCount(const TNavStatArray<FBox>& NavigableAreas, FVector::FReal TileSizeInWorldUnits, FVector::FReal AvgLayersPerGridCell, const uint32 NavMeshVersion)
{
	...
	const int64 MaxTilesCount = FMath::CeilToInt(static_cast<FVector::FReal>(GridCellsCount) * AvgLayersPerGridCell);

	// Output warning with details when overflowing
	UE_SUPPRESS(LogNavigationDataBuild, Warning,
	{
		if (!IntFitsIn<int32>(MaxTilesCount))
		{
			TStringBuilder<2048> DebugInformation;
			
			DebugInformation.Appendf(TEXT("%hs Computed maximum tile count is over limit. "
				"Check inputs for a abnormally high values.\n"
				"  MaxTilesCount: %llu, GridCellsCount: %llu, AvgLayersPerGridCell: %f, TileSizeInWorldUnits: %f \n"),
				__FUNCTION__, MaxTilesCount, GridCellsCount, AvgLayersPerGridCell, TileSizeInWorldUnits);

			DebugInformation.Appendf(TEXT("  Navigable areas:\n"));
			for (const FBox& AreaBounds : NavigableAreas)
			{
				DebugInformation.Appendf(TEXT("    AreaBounds: %s"), *AreaBounds.ToString());
			}
			
			UE_LOG(LogNavigationDataBuild, Warning, TEXT("%s"), DebugInformation.ToString());
		}
	});
	
	return IntCastChecked<int32>(FMath::Clamp(MaxTilesCount, 0, TNumericLimits<int32>::Max()));
}
} // UE::NavMesh::Private
...
void FRecastNavMeshGenerator::ConfigureBuildProperties(FRecastBuildConfig& OutConfig)
{
	...
	if (IsGeneratingLinks())
	{
		...
		check(OutConfig.JumpConfigs.IsEmpty());
		for (uint8 Index = 0; Index < DestNavMesh->NavLinkJumpConfigs.Num() && Index < TNumericLimits<uint8>::Max(); Index++)
		{
			const FNavLinkGenerationJumpConfig& JumpConfig = DestNavMesh->NavLinkJumpConfigs[Index];

			dtNavLinkBuilderJumpConfig& DetourJumpConfig = OutConfig.JumpConfigs.AddDefaulted_GetRef();
			JumpConfig.CopyToDetourConfig(DetourJumpConfig);
			DetourJumpConfig.ConfigIndex = Index;

			const float JumpSpillDistance = JumpConfig.bEnabled ? JumpConfig.JumpLength - JumpConfig.JumpDistanceFromEdge : 0.f;
			OutConfig.LinkSpillDistance = FMath::Max(JumpSpillDistance, OutConfig.LinkSpillDistance);
		}
	}
...
void FRecastNavMeshGenerator::ResolveGeneratedLinkAreas(FRecastBuildConfig& OutConfig)
{
	if (IsGeneratingLinks())
	{
		auto GetAreaIDAndPolyFlags = [this](const TSubclassOf<UNavAreaBase>& AreaClass, unsigned char& OutAreaID, ARecastNavMesh::FNavPolyFlags& OutPolyFlags)
		{
			if (AreaClass)
			{
				...
				if (AreaIDPtr != nullptr)
				{
					...
					// Using FlagsPerOffMeshLinkArea rather than FlagsPerArea because this function, ResolveGeneratedLinkAreas, 
					// is concerned only with navlinks (a.k.a. off-mesh links).
					OutPolyFlags = AdditionalCachedData.FlagsPerOffMeshLinkArea[*AreaIDPtr];
				}
				else
				{
		...
		if (ensureMsgf(DestNavMesh->NavLinkJumpConfigs.Num() == OutConfig.JumpConfigs.Num(), TEXT("Link configuration sizes are expected to match.")))
		{
			for (int32 Index = 0; Index < DestNavMesh->NavLinkJumpConfigs.Num(); Index++)
			{
				const FNavLinkGenerationJumpConfig& JumpConfig = DestNavMesh->NavLinkJumpConfigs[Index];
				dtNavLinkBuilderJumpConfig& DetourJumpConfig = OutConfig.JumpConfigs[Index];

				GetAreaIDAndPolyFlags(JumpConfig.DownDirectionAreaClass, DetourJumpConfig.downDirArea, DetourJumpConfig.downDirPolyFlag);
				GetAreaIDAndPolyFlags(JumpConfig.UpDirectionAreaClass, DetourJumpConfig.upDirArea, DetourJumpConfig.upDirPolyFlag);

				if (DetourJumpConfig.downDirArea == RECAST_NULL_AREA && DetourJumpConfig.upDirArea == RECAST_NULL_AREA)
				{
					...
					DetourJumpConfig.enabled = false;
				}
			}
		}
	}

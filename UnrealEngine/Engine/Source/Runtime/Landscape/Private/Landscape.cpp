...
void ALandscapeProxy::FixupSharedData(ALandscape* Landscape, const bool bMapCheck)
{
	...
	const bool bIsLandscapeInfoReady = LandscapeInfo->LandscapeActor != nullptr;
	...
	if (const FLandscapeTargetLayerSettings* InvalidTargetLayerSettings = TargetLayers.Find(FName()))
	{
		...
		if (bIsLandscapeInfoReady)
		{
			LandscapeInfo->DeleteLayer(InvalidTargetLayerSettings->LayerInfoObj, FName());
		}
		else
		{
			// Let each proxy handle target layer removal when the landscape info is not ready
			RemoveTargetLayer(FName(), /*bPostEditChange = */false);
		}
		bUpdated |= true;
	...
	if (bUpdated)
	{
		...
		const ALandscape* LandscapeActorOverride = bIsLandscapeInfoReady ? nullptr : Landscape;
		...
		bool bNeedsManualResave = LandscapeInfo->MarkObjectDirty(/*InObject = */this, /*bInForceResave = */true, LandscapeActorOverride);

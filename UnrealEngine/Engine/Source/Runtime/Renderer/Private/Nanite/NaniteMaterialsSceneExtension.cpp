		...
		SceneData->TaskHandles[InitPrimitiveDataTask] = GraphBuilder.AddSetupTask(
			[this]
			{
				for (auto PrimitiveSceneInfo : AddedList)
				{
					...
					NewData.bHasUVDensities = NaniteProxy->HasDynamicDisplacement() || NaniteProxy->HasVoxels();
	...
	const TConstArrayView<FPrimitiveSceneInfo*>& SceneInfosWithStaticDrawListUpdate)
{
	...
	auto UploadMaterialData = [this](const FPrimitiveData& Data)
	{
		...
		check(IsAligned(OutputBase, alignof(FNaniteMaterialSlot::FPacked)));
		...
		for (auto& PassMaterialSlots : Data.PrimitiveSceneInfo->NaniteMaterialSlots)
		{
			...
			{
				uint32 SectionIndex = 0;
				uint64 UninitializedSlots = (1ull << uint64(NumMaterials)) - 1ull;
				for (auto& MaterialSlot : PassMaterialSlots)
				{
					const uint64 MaterialIndex = MaterialSections[SectionIndex].MaterialIndex;
					MaterialSlotsOut[MaterialIndex] = MaterialSlot.Pack();
					UninitializedSlots &= ~(1ull << MaterialIndex);
					SectionIndex++;
				}

				// Initialize any uninitialized material slots.
				// NOTE: Getting into this loop means at least one material slot doesn't have a mesh section for it.
				// TODO: When the root cause of this is tracked down and fixed in build, we should just assert here. We should
				// also detect this sooner in scene proxy creation when building material sections to report the problem asset
				// and explore adding material sections with default settings appropriate for the material so we don't have to be
				// fault tolerant at this level
				//check(UninitializedSlots == 0);
				while (UninitializedSlots != 0)
				{
					const uint64 MaterialIndex = FMath::CountTrailingZeros64(UninitializedSlots);
					MaterialSlotsOut[MaterialIndex] = FNaniteMaterialSlot().Pack();
					UninitializedSlots &= ~(1ull << MaterialIndex);
				}
		...
		if (Data.bHasUVDensities)
		{
			...
			auto* UVDensitiesOut = reinterpret_cast<uint8*>(MaterialSlotsOut);
			check(IsAligned(UVDensitiesOut, alignof(float)));
			for (auto& MaterialSection : MaterialSections)
			{
				const uint32 MaterialDensitiesSize = sizeof(MaterialSection.LocalUVDensities);
				FMemory::Memcpy(&UVDensitiesOut[MaterialSection.MaterialIndex * MaterialDensitiesSize], &MaterialSection.LocalUVDensities, MaterialDensitiesSize);
			}
		}
	};
...
{
	// Not guaranteed because the condition may change _after_ the FRenderer is created
	if (!SceneData->IsEnabled())
	{
		return;
	}

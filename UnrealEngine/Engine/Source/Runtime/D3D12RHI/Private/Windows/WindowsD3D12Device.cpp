...
void FD3D12DynamicRHI::Init()
{
	...
	if (IsRHIDeviceNVIDIA())
	{
		...
		{
			UE_LOG(LogD3D12RHI, Error, TEXT("Couldn't find CVar r.D3D12.Submission.MaxExecuteBatchSize.Async to work around a known bug in certain drivers."));
		}
		else if (FDriverVersion(GRHIGlobals.GpuInfo.AdapterUserDriverVersion) < FDriverVersion(TEXT("571.23")))
		{
			CVarD3D12SubmissionAsyncMaxExecuteBatchSize->Set(1);
			UE_LOG(LogD3D12RHI, Warning, TEXT("Batched command list execution is disabled for async queues due to known bugs in the current driver."));
		}
	} // if NVIDIA

...
struct FSequencerSectionPainterImpl : FSequencerSectionPainter
{
	...
	void DrawBlendType()
	{
		...
		UEnum* Enum = FindObjectChecked<UEnum>(nullptr, TEXT("/Script/MovieScene.EMovieSceneBlendType"), EFindObjectFlags::ExactClass);
...
void SSequencerSection::Tick( const FGeometry& AllottedGeometry, const double InCurrentTime, const float InDeltaTime )
{
	...
	if (!Section || !SequencerPtr || !SectionModel || !MovieScene)
	{
		return;
	}
	...
	if (GetVisibility() == EVisibility::Visible)
	{
		if (Section->HasStartFrame() && Section->HasEndFrame())
		{
			...
			FTimeToPixel TimeToPixelConverter(ParentGeometry, GetSequencer().GetViewRange(), MovieScene->GetTickResolution());

	...
	, const TArray<FOnCreateOutlinerIndicator>& OutlinerIndicatorDelegates)
{
	...
	{
		Settings->SetLoopMode(SLM_LoopSelectionRange);
	}
...
FSequencer::~FSequencer()
{
	// Suppress any deferred modification flushes during destruction.
	// This prevents view model updates from trying to access the dying Sequencer
	// when the deferred flush is triggered during teardown.

	UE::MovieScene::FScopedSignedObjectModifySuppress SuppressFlush;
	if (Runner)
	{
...
void FSequencer::GetConstructionScriptActors(UMovieScene* MovieScene, FMovieSceneSequenceIDRef SequenceID, TSet<TWeakObjectPtr<AActor> >& BoundActors, TArray < TPair<FMovieSceneSequenceID, FGuid> >& BoundGuids)
{
	...
	for (int32 Index = 0; Index < MovieScene->GetPossessableCount(); ++Index)
	{
		...
		FMovieSceneBinding* MovieSceneBinding = MovieScene->FindBinding(ThisGuid);
...
void FSequencer::SelectInSelectionRange(const TSharedPtr<UE::Sequencer::FViewModel>& Item, const TRange<FFrameNumber>& SelectionRange, bool bSelectKeys, bool bSelectSections)
{
	...
	if (bSelectKeys)
	{
		...
		for (TSharedPtr<FChannelModel> Channel : Channels)
		{
			TSharedPtr<IKeyArea> KeyArea = Channel->GetKeyArea();
...
void FSequencer::UpdatePlaybackRange()
{
	...
	if (AllSections.Num() > 0 && !IsPlaybackRangeLocked())
	{
		...
		if (!NewBounds.IsDegenerate() && !NewBounds.IsEmpty())
		{
...
FGuid FSequencer::GetHandleToObject( UObject* Object, bool bCreateHandleIfMissing, const FName& CreatedFolderName )
{
	...
	if (TSharedPtr<IObjectSchema> Schema = Module.FindObjectSchema(Object))
	{
		Object = Schema->GetRedirectedBoundObject(Object);
		if (!Object)
		{
			return FGuid();
		}
...
FReply FSequencer::OnJumpToStart()
{
	...
	const bool bWasPlaying = Settings->GetStopPlayingWhenJumpingToStartOrEnd() == false && (GetPlaybackStatus() == EMovieScenePlayerStatus::Playing);
...
FString FSequencer::GetFrameTimeText() const
{
	...
	FFrameTime LocalTime = bWarpedTime ? GlobalPlaybackWarpTransform.TransformTime(GetGlobalTime().Time) : GetGlobalTime().Time;
	...
	if (ScrubPositionParent.IsSet() && ScrubPositionParent.GetValue() != MovieSceneSequenceID::Root)
	{
		...
		LocalTime = RootToParentChainTransform->TransformTime(GetGlobalTime().Time, FTransformTimeParams().IgnoreClamps());
	}
...
{
	if (InScrubPositionParent == GetFocusedTemplateID())
	{
		ScrubPositionParent.Reset();
	}
	else
...
void FSequencer::HandleSelectedOutlinerNodesChanged()
{
	...
	OnSelectionChangedObjectGuidsDelegate.Broadcast(ViewModel->GetSelection()->GetBoundObjectsGuids().Array());
...
{
	Objects = GetSelection().GetBoundObjectsGuids().Array();
}
...
void FSequencer::DeselectObject(FGuid ObjectBinding)
{
	using namespace UE::Sequencer;
	...
	if (TSharedPtr<FObjectBindingModel> Model = ObjectStorage->FindModelForObjectBinding(ObjectBinding))
	{
		ViewModel->GetSelection()->Outliner.Deselect(Model);
	}
}
...
void FixSortingOrders(FMovieSceneBinding* InBinding, const UMovieScene* MovieScene)
{
	// Fix the sorting orders if InBinding is going to conflict with any of the existing bindings (ie. same class and same name)
	TOptional<int32> MaxSortingOrder;
	FText InBindingName = MovieScene->GetObjectDisplayName(InBinding->GetObjectGuid());
	for (const FMovieSceneBinding& Binding : MovieScene->GetBindings())
	{
		if (&Binding != InBinding && MovieScene->GetObjectDisplayName(Binding.GetObjectGuid()).EqualTo(InBindingName))
		{
			if (MaxSortingOrder.IsSet())
			{
...
void FSequencer::CopySelection()
{
	...
	else
	{
		...
		{
			CopySelectedFolders(FoldersToCopy, FoldersExportedText, ObjectsExportedText, TracksInFoldersExportedText);
		}
...
void FSequencer::BindCommands()
{
	...
	SequencerCommandBindings->MapAction(
		Commands.ToggleLoopCuts,
		FExecuteAction::CreateLambda( [this]{ this->ToggleLoopCuts(); } ),
		FCanExecuteAction::CreateLambda( []{ return true; } ),
		FIsActionChecked::CreateLambda( [this]{ return this->UsesLoopCuts(); } ) );
	...
	SequencerCommandBindings->MapAction(
		...
		FCanExecuteAction(),
		FIsActionChecked(),
		FIsActionButtonVisible::CreateLambda([this] { return Cast<ULevelSequence>(GetFocusedMovieSceneSequence()) != nullptr; })
	);
...
void FSequencer::ToggleDynamicWeighting()
{
	...
	FScopedTransaction Transaction(EnumHasAnyFlags(NewFlags, EMovieSceneSequenceFlags::DynamicWeighting) ? LOCTEXT("EnableDynamicWeighting", "Enable Dynamic Weighting") : LOCTEXT("DisableDynamicWeighting", "Disable Dynamic Weighting"));
...
void FSequencer::ToggleLoopCuts()
{
	UMovieSceneSequence* Sequence = GetFocusedMovieSceneSequence();

	EMovieSceneSequenceFlags NewFlags = Sequence->GetFlags();
	NewFlags ^= EMovieSceneSequenceFlags::LoopCuts;

	FScopedTransaction Transaction(EnumHasAnyFlags(NewFlags, EMovieSceneSequenceFlags::LoopCuts) ? LOCTEXT("EnableLoopCuts", "Enable Loop Cuts") : LOCTEXT("DisableLoopCuts", "Disable Loop Cuts"));

	Sequence->Modify();
	Sequence->SetSequenceFlags(NewFlags);
}

bool FSequencer::UsesLoopCuts()
{
	return EnumHasAnyFlags(GetFocusedMovieSceneSequence()->GetFlags(), EMovieSceneSequenceFlags::LoopCuts);
}
